...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-(--card-spacing) rounded-card border border-border bg-card py-(--card-spacing) text-sm text-card-foreground shadow-card transition-all duration-200 ease-brand hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-card-hover [--card-spacing:--spacing(4)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(3)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-card *:[img:last-child]:rounded-b-card",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("h-card group-data-[size=sm]/card:text-sm", className)}
      {...props}
    />
  )
}

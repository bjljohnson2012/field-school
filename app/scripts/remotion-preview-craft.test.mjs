...
test("Remotion plates record a soft note only when audio drifts from the timeline or caption cues", () => {
  const checker = readFileSync(join(root, "..", "plates", "scripts", "remotion-soft-craft-notes.mjs"), "utf8");
  const spoken = [
    { text: "Slate.", startMs: 10000, endMs: 10400 },
    { text: "Household:", startMs: 10400, endMs: 11200 },
  ...
  const synced = remotionSoftCraftNotes({ cues: [cue], spoken, chapters, audio: [audio], cleaningFlip: true });
  assert.deepEqual(synced.notes, []);
  assert.equal(synced.cleaningFlip, false);
  assert.equal(synced.holdCleaning, true);

  const timeline = remotionSoftCraftNotes({
    cues: [cue],
    spoken,
    chapters,
    audio: [{ ...audio, startMs: 10400 }],
  });
  assert.equal(timeline.notes.length, 1);
  assert.equal(timeline.notes[0].kind, "audio-desync");
  assert.equal(timeline.notes[0].timelineStartMs, 10000);
  assert.equal(timeline.notes[0].audioStartMs, 10400);

  const cueDrift = remotionSoftCraftNotes({
    cues: [cue],
    spoken,
    chapters,
    audio: [{ ...audio, cueEndMs: 14800 }],
  });
  assert.equal(cueDrift.notes.length, 1);
  assert.equal(cueDrift.notes[0].kind, "audio-desync");
  assert.equal(cueDrift.notes[0].cueEndMs, 14800);
...
test("Remotion plates record a soft note only for flicker or a flash pattern", () => {
  const checker = readFileSync(join(root, "..", "plates", "scripts", "remotion-soft-craft-notes.mjs"), "utf8");
  const opener = readFileSync(join(root, "..", "plates", "src", "Opener.tsx"), "utf8");
  const spoken = [
    { text: "Slate.", startMs: 10000, endMs: 10400 },
  ...
  const held = (value, count) => Array.from({ length: count }, () => value);
  const quiet = remotionSoftCraftNotes({
    ...
    flicker: [
      { id: "fade", luma: Array.from({ length: 20 }, (_, index) => index / 19) },
  ...
  const frame = remotionSoftCraftNotes({
    cues: [cue],
    spoken,
    chapters,
    audio: [audio],
  ...
  assert.deepEqual(frame.notes, [{ kind: "flicker", id: "slate", pattern: "frame" }]);

  const flash = remotionSoftCraftNotes({
    cues: [cue],
    spoken,
    chapters,
    audio: [audio],
    compositions: [{ id: "Opener", source: opener }],
    durations: [{ id: "LessonSpine", seconds: 41 }],
    flicker: [{ id: "slate", luma: [...held(0, 8), ...held(1, 8), ...held(0, 8), ...held(1, 8), ...held(0, 8)] }],
  });
  assert.deepEqual(flash.notes, [{ kind: "flicker", id: "slate", pattern: "flash" }]);
...
test("Remotion plates record a soft note only when a beat teaches more than one objective", () => {
  const checker = readFileSync(join(root, "..", "plates", "scripts", "remotion-soft-craft-notes.mjs"), "utf8");
  const opener = readFileSync(join(root, "..", "plates", "src", "Opener.tsx"), "utf8");
  const spoken = [
    { text: "Slate.", startMs: 10000, endMs: 10400 },
  ...
  const quiet = remotionSoftCraftNotes({
    ...
    durations: [{ id: "LessonSpine", seconds: spineDurationSec(SPINE_BEATS) }],
    flicker: [{ id: "cut", luma: [0, 0, 0, 1, 1, 1] }],
    contrast: [{ id: "body", role: "text", foreground: "#1A1A16", background: "#EFE7D6" }],
    beats: [{ id: "objective", objective: "Name the type." }, splitQuiz],
    cleaningFlip: true,
  });
  assert.deepEqual(quiet.notes, []);
  assert.equal(quiet.cleaningFlip, false);
  ...
  const piled = remotionSoftCraftNotes({
    cues: [cue],
    spoken,
    chapters,
    audio: [audio],
  ...
  assert.deepEqual(piled.notes, [{ kind: "multi-objective", id: "slate", count: 2 }]);

  const segment = remotionSoftCraftNotes({
    cues: [cue],
    spoken,
    chapters,
    audio: [audio],
  ...
  assert.deepEqual(segment.notes, [{ kind: "multi-objective", id: "quiz", count: 2 }]);
...
test("signed-in Remotion rail matches the guest b-roll credit when the plate requests it", () => {
  ...
  assert.notEqual(signedIn["data-live-broll"], "absent");
  assert.equal(signedIn["data-live-broll"], "non-null");
  assert.notEqual(signedIn["data-pexels-rail-attribution"], "absent");
  assert.equal(signedIn["data-pexels-rail-attribution"], "present");
  assert.notEqual(signedIn["data-pexels-credit"], "absent");
  assert.equal(signedIn["data-pexels-credit"], "live");
  assert.equal(signedIn["data-pexels-rail-attribution"], guest.state);
  const beforeFetch = signedInRemotionBrollMarkers(null, "remotion");
  assert.ok(beforeFetch);
  assert.equal(beforeFetch["data-broll-request"], "classroom");
  assert.notEqual(beforeFetch["data-live-broll"], "absent");
  assert.equal(beforeFetch["data-live-broll"], "non-null");
  assert.notEqual(beforeFetch["data-pexels-rail-attribution"], "absent");
  assert.equal(beforeFetch["data-pexels-rail-attribution"], "present");
  assert.notEqual(beforeFetch["data-pexels-credit"], "absent");
  assert.equal(beforeFetch["data-pexels-credit"], "live");
  assert.equal(signedInRemotionBrollMarkers(mounted, null), null);
  assert.equal(signedInRemotionBrollMarkers(mounted, "html5"), null);
  const partial = signedInRemotionBrollMarkers({ photographer: "Ada Frame" }, "remotion");
  assert.ok(partial);
  assert.notEqual(partial["data-live-broll"], "absent");
  assert.equal(partial["data-live-broll"], "non-null");
  assert.equal(partial["data-pexels-rail-attribution"], "present");
  assert.equal(partial["data-pexels-credit"], "live");

...
import {
  ...
  sliceHirePathPortion,
  writeSupervisedPortion,
} from "../src/lib/progress/supervised-portion.ts";
...
test("Next slice under the selected Child from assembled path and parent intent", () => {
  const play = selectSupervisedPortion("play-child", {
    horizon: "this hire",
    pathItems: [
      { title: "LessonSpine Ready / HLS", subject: "LessonSpine", play: "/play/lesson-spine" },
      { title: "Hold LessonSpine from parent intent", subject: "LessonSpine", play: "/play/lesson-spine" },
      { title: "parent-owned plan through LessonSpine", subject: "LessonSpine", play: "/progress" },
    ],
  });
  ...
  assert.equal(play.selected?.status, "suggested");
  assert.equal(play.selected?.horizon, "this hire");
  assert.match(play.selected?.items[0]?.title || "", /LessonSpine/);
  const sliced = sliceHirePathPortion(
    [{ title: "LessonSpine Ready / HLS" }, { title: "Hold LessonSpine from parent intent" }],
    "this hire",
  );
  assert.equal(sliced[0].title, "LessonSpine Ready / HLS");
  ...
  const unknown = writeSupervisedPortion("not-a-child", "lock", {
    pathItems: [{ title: "no" }],
  });
...
test("FR-5 campus surface does not steal family LIVE or portion store", () => {
  const page = read("src/app/portion/page.tsx");
  const ui = read("src/components/fr-5-parent-portion.tsx");
  const api = read("src/app/api/progress/portion/route.ts");
  const lib = read("src/lib/progress/supervised-portion.ts");
  ...
  assert.match(page, /Fr5ParentPortion/);
  assert.match(ui, /data-portion="fr-5"/);
  assert.match(ui, /data-selected-child/);
  assert.match(ui, /\/api\/progress\/portion/);
  assert.match(api, /writeSupervisedPortion/);
  assert.match(lib, /FR-5/);
  assert.doesNotMatch(childrenDb, /Now \/ Confidence \/ Next/);
  assert.doesNotMatch(childrenPage, /Now \/ Confidence \/ Next/);
  assert.doesNotMatch(
    ui + lib + api,
    /family-v1-home|children-database|bc-4765f2f0|\/api\/curriculum|curriculum_paths|\/api\/intent|learning_intents|next_portions|next_portion_items/,
  );
...
test("master and priors untouched after parent next-portion", () => {
...
test("WAVE5 STATUS cite Parent next-portion after PR 196 tip; launch CLOSED 0/8", () => {
  ...
  assert.match(wave5, /## Parent next-portion \(PASS\)/);
  assert.match(wave5, /PR 196 tip `dbd57dc`/);
  assert.match(wave5, /\/portion/);
  ...
  assert.match(campus, /PR 196 tip `dbd57dc`/);
  ...
  assert.match(rail, /\/portion/);
  assert.match(rail, /Parent next-portion/);
  assert.match(gate, /Product \| HELD/);
  assert.match(gate, /Parent next-portion FR-5/);

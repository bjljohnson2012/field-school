...
if [ -f "$VO" ]; then
  ffmpeg -y -hide_banner -loglevel error -stream_loop 2 -t 64 -i "$SRC" -i "$VO" \
    -filter_complex "[0:a]${SCOOP}[bed];[1:a]adelay=6000,apad=whole_dur=64,aformat=channel_layouts=stereo[sc];[bed][sc]sidechaincompress=threshold=0.055:ratio=2.6:attack=70:release=480:knee=10:level_sc=1:mix=0.8" \

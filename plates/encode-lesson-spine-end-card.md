# LessonSpine re-encode with EndCard

Locked pedagogical order: Opener/sting → TalkingHead/slate → DefinitionBoard/objective → RecapCard → QuizBumper/next-up.

New dated dest only. Bundles PR 118 EndCard lesson close signal onto the transition-luma master stack (PR 85–117). Factory TypeCard + AudioBed + Letterbox + OverlayLock + Karaoke packing + ProgressRail + ChapterChip + LowerThird + CalloutCard + TransitionLuma + EndCard. LessonSpine objective beat stays DefinitionBoard. No Cap A-roll.
...
```bash
mkdir -p /opt/cursor/artifacts/lesson-spine-end-card-encode/2026-09-20
node scripts/render-plate.mjs \
  --comp LessonSpine \
  --dest /opt/cursor/artifacts/lesson-spine-end-card-encode/2026-09-20/LessonSpine.mp4 \

# LessonSpine re-encode with ScriptureCard

Locked pedagogical order: Opener/sting → TalkingHead/slate → DefinitionBoard/objective → RecapCard → QuizBumper/next-up.

New dated dest only. Bundles PR 127 ScriptureCard verse / citation lock onto the KeyClaim master stack (PR 85–126). Factory TypeCard + AudioBed + Letterbox + OverlayLock + Karaoke packing + ProgressRail + ChapterChip + LowerThird + CalloutCard + TransitionLuma + EndCard + PracticeCard + KeyClaim + ScriptureCard. LessonSpine objective beat stays DefinitionBoard. No Cap A-roll.
...
```bash
mkdir -p /opt/cursor/artifacts/lesson-spine-scripture-encode/2026-09-21
node scripts/render-plate.mjs \
  --comp LessonSpine \
  --dest /opt/cursor/artifacts/lesson-spine-scripture-encode/2026-09-21/LessonSpine.mp4 \

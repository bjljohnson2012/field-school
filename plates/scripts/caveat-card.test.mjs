...
const MASTER_SHA = "e0ead459d9505cd612403e8601ab1bfd9ef1acbfc3eba59d3640e19ce621935b";
const STILLS = "/opt/cursor/artifacts/remotion-caveat-card/2026-09-21";
const PRIOR =
  "/opt/cursor/artifacts/lesson-spine-quote-encode/2026-09-21/LessonSpine.mp4";
const PRIOR_SHA = "244c485bbfd83e2138c4e499fc07a00f566be3d1ede1c5a3f03dfd57b04feb2d";
...
test("CaveatCard is a watch-out / warning after sting/objective path", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "CaveatCardDemo.tsx");
  const recap = src("src", "RecapCard.tsx");
  const board = src("src", "DefinitionBoard.tsx");
  assert.match(layers, /export function CaveatCard/);
  assert.match(layers, /export const CAVEAT_CARD_KICKER/);
  assert.match(layers, /export const CAVEAT_CARD_LINE/);
  assert.match(layers, /CAVEAT_CARD_KICKER = "Caveat"/);
  assert.match(layers, /CAVEAT_CARD_LINE = "Watch the catch\."/);
  assert.match(layers, /Watch-out \/ warning/);
  assert.match(layers, /After sting\/objective path, after steps, before practice/);
  ...
  assert.match(demo, /CaveatCardDemo/);
  assert.match(demo, /name="caveat"/);
...
test("CaveatCardDemo registers after StepsCardDemo; Recap/Definition sequence caveat after steps before practice", () => {
  ...
  assert.ok(rootTsx.indexOf('id="StepsCardDemo"') < rootTsx.indexOf('id="CaveatCardDemo"'));
  assert.ok(rootTsx.indexOf('id="CaveatCardDemo"') < rootTsx.indexOf('id="LessonSpine"'));
  assert.doesNotMatch(opener, /CaveatCard|name="caveat"/);
  assert.doesNotMatch(head, /CaveatCard|name="caveat"/);
  assert.doesNotMatch(quiz, /CaveatCard|name="caveat"/);
  ...
  assert.ok(
    boardWipe >= 0 &&
      boardSteps > boardWipe &&
      boardCaveat > boardSteps &&
...
test("ORDER LOCK intact; docs seal CaveatCard PASS with hold_cleaning", () => {
  const note = src("antagonist-caveat-card.md");
  ...
  assert.match(note, /e0ead459d9505cd612403e8601ab1bfd9ef1acbfc3eba59d3640e19ce621935b/);
  assert.match(note, /PR 155 merge `0b0fe4dd4fe4e69797c5d278f1e0afcfae6963b9`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## CaveatCard 2026-09-21/);
  assert.match(full, /Dated \*\*2026-09-21\*\* reaudit/);
  assert.match(wave5, /## CaveatCard \(PASS\)/);
  assert.match(wave5, /PR 155 merge `0b0fe4d`/);
  assert.match(wave5, /e0ead459d9505cd612403e8601ab1bfd9ef1acbfc3eba59d3640e19ce621935b/);
  assert.match(wave5, /\*\*Current master dest\.\*\*/);
  assert.match(campus, /PR 155 merge `0b0fe4d`/);
  assert.match(campus, /CaveatCard \*\*PASS\*\*/);
  assert.match(campus, /e0ead459d9505cd612403e8601ab1bfd9ef1acbfc3eba59d3640e19ce621935b/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-caveat-card\.md/);
  assert.match(src("AGENTS.md"), /CaveatCardDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of [
    "CaveatCardDemo-f0.png",
    "CaveatCardDemo-f60.png",
...
test("checklist on steps-encode master holds; --flip refused", () => {

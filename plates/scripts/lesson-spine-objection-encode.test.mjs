...
const NEW_SHA = "4a1317e0e6daf440958a81a7fb6dcda042751c9fda3ab44f1df1153d8fd66aee";
const STILLS = "/opt/cursor/artifacts/remotion-lesson-spine-objection-encode/2026-09-21";
const PRIOR =
...
test("WAVE5 and STATUS promote objection encode as current master dest", () => {
  const wave5 = src("..", "docs", "campus-runtime", "WAVE5.md");
  const campus = src("..", "docs", "campus-runtime", "STATUS.md");
  const note = src("antagonist-lesson-spine-objection-encode.md");
  assert.match(
    wave5,
    /Current master LessonSpine dest: `\/opt\/cursor\/artifacts\/lesson-spine-objection-encode\/2026-09-21\/LessonSpine\.mp4`/,
  );
  assert.match(wave5, /4a1317e0e6daf440958a81a7fb6dcda042751c9fda3ab44f1df1153d8fd66aee/);
  assert.match(wave5, /## LessonSpine objection encode \(PASS\)/);
  assert.match(wave5, /PRs 85–139/);
  assert.match(wave5, /PR 139 merge `19e4d28`/);
  assert.match(wave5, /247890d0529b635d8ad39317825c760fe76fe134274e2149e4ccbe9bcb1af32c/);
  assert.match(wave5, /Archived prior glossary-encode/);
  assert.match(wave5, /\*\*Current master dest\.\*\*/);
  assert.match(campus, /Current master LessonSpine dest is objection-encode/);
  assert.match(campus, /4a1317e0e6daf440958a81a7fb6dcda042751c9fda3ab44f1df1153d8fd66aee/);
  assert.match(campus, /PR 139 merge `19e4d28`/);
  assert.match(campus, /Archived prior glossary-encode/);
  ...
  assert.match(note, /PR 139 merge `19e4d28225f89cc3ab36d35817355a62382223a8`/);
  ...
  assert.match(src("README.md"), /antagonist-lesson-spine-objection-encode\.md/);
...
test("new dest exists; glossary-encode and other priors untouched", () => {
...
test("render-lock dry-run forwards the new dest and refuses Just", () => {
  const dir = mkdtempSync(join(tmpdir(), "objection-encode-lock-"));

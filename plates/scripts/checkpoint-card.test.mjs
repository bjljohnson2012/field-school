...
const MASTER_SHA = "325a41e775821c8576e14038d2076ee639d48a0fc29b3f84b223f4af9390f204";
const STILLS = "/opt/cursor/artifacts/remotion-checkpoint-card/2026-09-21";
const PRIOR =
  "/opt/cursor/artifacts/lesson-spine-objection-encode/2026-09-21/LessonSpine.mp4";
const PRIOR_SHA = "4a1317e0e6daf440958a81a7fb6dcda042751c9fda3ab44f1df1153d8fd66aee";
...
test("CheckpointCard is a formative check / confirm-understanding after sting/objective path", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "CheckpointCardDemo.tsx");
  const recap = src("src", "RecapCard.tsx");
  const board = src("src", "DefinitionBoard.tsx");
  assert.match(layers, /export function CheckpointCard/);
  assert.match(layers, /export const CHECKPOINT_CARD_KICKER/);
  assert.match(layers, /export const CHECKPOINT_CARD_LINE/);
  assert.match(layers, /CHECKPOINT_CARD_KICKER = "Check"/);
  assert.match(layers, /CHECKPOINT_CARD_LINE = "Confirm the idea\."/);
  assert.match(layers, /Formative check \/ confirm-understanding/);
  assert.match(layers, /After sting\/objective path, after objection, before practice/);
  ...
  assert.match(demo, /CheckpointCardDemo/);
  assert.match(demo, /name="checkpoint"/);
...
test("CheckpointCardDemo registers after StingColdOpenDemo; Recap/Definition sequence checkpoint after objection before practice", () => {
  ...
  assert.ok(rootTsx.indexOf('id="StingColdOpenDemo"') < rootTsx.indexOf('id="CheckpointCardDemo"'));
  assert.ok(rootTsx.indexOf('id="CheckpointCardDemo"') < rootTsx.indexOf('id="LessonSpine"'));
  assert.doesNotMatch(opener, /CheckpointCard|name="checkpoint"/);
  assert.doesNotMatch(head, /CheckpointCard|name="checkpoint"/);
  assert.doesNotMatch(quiz, /CheckpointCard|name="checkpoint"/);
  ...
  assert.ok(
    boardWipe >= 0 &&
      boardObjection > boardWipe &&
      boardCheck > boardObjection &&
...
test("ORDER LOCK intact; docs seal CheckpointCard PASS with hold_cleaning", () => {
  const note = src("antagonist-checkpoint-card.md");
  ...
  assert.match(note, /325a41e775821c8576e14038d2076ee639d48a0fc29b3f84b223f4af9390f204/);
  assert.match(note, /PR 143 merge `1c73749362cfa0111095aa95eacb9564bf4e4232`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## CheckpointCard 2026-09-21/);
  assert.match(full, /Dated \*\*2026-09-21\*\* reaudit/);
  assert.match(wave5, /## CheckpointCard \(PASS\)/);
  assert.match(wave5, /PR 143 merge `1c73749`/);
  assert.match(wave5, /325a41e775821c8576e14038d2076ee639d48a0fc29b3f84b223f4af9390f204/);
  assert.match(wave5, /\*\*Current master dest\.\*\*/);
  assert.match(campus, /PR 143 merge `1c73749`/);
  assert.match(campus, /CheckpointCard \*\*PASS\*\*/);
  assert.match(campus, /325a41e775821c8576e14038d2076ee639d48a0fc29b3f84b223f4af9390f204/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-checkpoint-card\.md/);
  assert.match(src("AGENTS.md"), /CheckpointCardDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of [
    "CheckpointCardDemo-f0.png",
    "CheckpointCardDemo-f60.png",
...
test("checklist on sting-encode master holds; --flip refused", () => {

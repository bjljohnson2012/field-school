...
const MASTER_SHA = "cb3a672da3871428548f2e6ec69da03b9c1d2e425058a1b4b856d928a6e43a79";
const STILLS = "/opt/cursor/artifacts/remotion-compare-board/2026-09-21";
const PRIOR =
  "/opt/cursor/artifacts/lesson-spine-key-claim-encode/2026-09-21/LessonSpine.mp4";
const PRIOR_SHA = "16f516640c5cac4609862ae2436eb37dd118e56bc6633e3dd10d76290a588606";
...
test("CompareBoard is a two-claim / contrast lock after sting/objective path", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "CompareBoardDemo.tsx");
  assert.match(layers, /export function CompareBoard/);
  assert.match(layers, /export const COMPARE_BOARD_KICKER/);
  assert.match(layers, /export const COMPARE_BOARD_LINE/);
  assert.match(layers, /COMPARE_BOARD_KICKER = "Compare"/);
  assert.match(layers, /COMPARE_BOARD_LINE = "Hold two claims\."/);
  assert.match(layers, /COMPARE_BOARD_LEFT = "Type holds\."/);
  assert.match(layers, /COMPARE_BOARD_RIGHT = "Head docks\."/);
  assert.match(layers, /Two-claim \/ contrast/);
  assert.match(layers, /After sting\/objective path, after scripture, before practice/);
  ...
  assert.match(demo, /CompareBoardDemo/);
  assert.match(demo, /name="compare"/);
  assert.match(demo, /<CompareBoard \/>/);
  assert.match(demo, /beat="objective"/);
  assert.doesNotMatch(demo, /a_roll\.mp4|27pn9xs0zk8a73g/);
});

test("CompareBoardDemo registers after ScriptureCardDemo; Opener/Definition/Recap sequence compare after scripture", () => {
  ...
  assert.ok(rootTsx.indexOf('id="ScriptureCardDemo"') < rootTsx.indexOf('id="CompareBoardDemo"'));
  assert.ok(rootTsx.indexOf('id="CompareBoardDemo"') < rootTsx.indexOf('id="LessonSpine"'));
  ...
  assert.ok(
    openerWipe >= 0 &&
      openerScripture > openerWipe &&
      openerCompare > openerScripture &&
      openerAudio > openerCompare,
  );
  assert.match(opener, /<CompareBoard \/>/);
  ...
  assert.ok(
    boardWipe >= 0 &&
      boardScripture > boardWipe &&
      boardCompare > boardScripture &&
      boardAudio > boardCompare,
  );
  assert.match(board, /<CompareBoard \/>/);
  ...
  assert.ok(
    recapWipe >= 0 &&
      recapScripture > recapWipe &&
      recapCompare > recapScripture &&
      recapPractice > recapCompare &&
...
test("ORDER LOCK intact; docs seal CompareBoard PASS with hold_cleaning", () => {
  const note = src("antagonist-compare-board.md");
  ...
  assert.match(note, /cb3a672da3871428548f2e6ec69da03b9c1d2e425058a1b4b856d928a6e43a79/);
  assert.match(note, /PR 129 merge `504595b435915b926c94233291c091a2653c8441`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## CompareBoard 2026-09-21/);
  assert.match(full, /Dated \*\*2026-09-21\*\* reaudit/);
  assert.match(wave5, /## CompareBoard \(PASS\)/);
  assert.match(wave5, /PR 129 merge `504595b`/);
  assert.match(wave5, /cb3a672da3871428548f2e6ec69da03b9c1d2e425058a1b4b856d928a6e43a79/);
  assert.match(wave5, /\*\*Current master dest\.\*\*/);
  assert.match(campus, /PR 129 merge `504595b`/);
  assert.match(campus, /CompareBoard \*\*PASS\*\*/);
  assert.match(campus, /cb3a672da3871428548f2e6ec69da03b9c1d2e425058a1b4b856d928a6e43a79/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-compare-board\.md/);
  assert.match(src("AGENTS.md"), /CompareBoardDemo/);
...
test("checklist on scripture-encode master holds; --flip refused", () => {

...
const STILLS = "/opt/cursor/artifacts/remotion-antagonist-threshold-encode-master/2026-09-21";
const PRIOR =
...
test("full-set threshold encode master reaudit is dated PASS with hold_cleaning", () => {
  ...
  assert.match(full, /## LessonSpine threshold encode master dest reaudit 2026-09-21/);
  assert.match(full, /955f0256424fdc4ddb5ae5506a04a9ea877cb6a79ccb126ba1bb4884b9943a6f/);
  ...
  assert.match(src("antagonist-threshold-encode-master.md"), /EDU-S01: PASS/);
  assert.match(src("antagonist-threshold-encode-master.md"), /EDU-S02: PASS/);
  assert.match(
    src("antagonist-threshold-encode-master.md"),
    /955f0256424fdc4ddb5ae5506a04a9ea877cb6a79ccb126ba1bb4884b9943a6f/,
  );
  assert.match(
    src("antagonist-threshold-encode-master.md"),
    /PR 172 merge `e58a798acc98cf307b180234bfa171aaaeecc8f8`/,
  );
  assert.match(wave5, /## LessonSpine threshold encode master dest reaudit \(PASS\)/);
  assert.match(wave5, /PR 172 merge `e58a798`/);
  ...
  assert.match(campus, /PR 172 merge `e58a798`/);
  assert.match(campus, /LessonSpine threshold encode master dest reaudit \*\*PASS\*\*/);
  assert.match(campus, /955f0256424fdc4ddb5ae5506a04a9ea877cb6a79ccb126ba1bb4884b9943a6f/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-threshold-encode-master\.md/);
  assert.doesNotMatch(wave5, /8\/8 PASS|launch OPEN/i);
});

test("locked threshold encode master dest and audit stills exist; priors untouched", () => {
  ...
  assert.equal(sha256(TIMELINE), TIMELINE_SHA);

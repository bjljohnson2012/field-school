...
const MASTER_SHA = "f4bf345d6c0ec8c51941b3bb2d08c71589bc822632d935dc26b6d18c07a631e7";
const STILLS = "/opt/cursor/artifacts/remotion-glossary-chip/2026-09-21";
const PRIOR =
  "/opt/cursor/artifacts/lesson-spine-compare-encode/2026-09-21/LessonSpine.mp4";
const PRIOR_SHA = "917efdb18684f7307ec63dd576b26f15eea207194ea9a6804d121878ff3f3a5d";
...
test("GlossaryChip is a term / definition pop after sting/objective path", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "GlossaryChipDemo.tsx");
  assert.match(layers, /export function GlossaryChip/);
  assert.match(layers, /export const GLOSSARY_CHIP_KICKER/);
  assert.match(layers, /export const GLOSSARY_CHIP_TERM/);
  assert.match(layers, /export const GLOSSARY_CHIP_LINE/);
  assert.match(layers, /GLOSSARY_CHIP_KICKER = "Glossary"/);
  assert.match(layers, /GLOSSARY_CHIP_TERM = "Type"/);
  assert.match(layers, /GLOSSARY_CHIP_LINE = "Define the term\."/);
  assert.match(layers, /Term \/ definition pop/);
  assert.match(layers, /After sting\/objective path, after section, before practice/);
  ...
  assert.match(demo, /GlossaryChipDemo/);
  assert.match(demo, /name="glossary"/);
  assert.match(demo, /<GlossaryChip \/>/);
  assert.match(demo, /beat="objective"/);
  assert.doesNotMatch(demo, /a_roll\.mp4|27pn9xs0zk8a73g/);
});

test("GlossaryChipDemo registers after SectionTitleDemo; Opener/Definition/Recap sequence glossary after section", () => {
  ...
  assert.ok(
    boardWipe >= 0 &&
      boardSection > boardWipe &&
      boardGlossary > boardSection &&
      boardAudio > boardGlossary,
  );
...
test("ORDER LOCK intact; docs seal GlossaryChip PASS with hold_cleaning", () => {
  const note = src("antagonist-glossary-chip.md");
  ...
  assert.match(note, /f4bf345d6c0ec8c51941b3bb2d08c71589bc822632d935dc26b6d18c07a631e7/);
  assert.match(note, /PR 135 merge `a7facba844e00865d330bcd8ea812befdc5b9ab1`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## GlossaryChip 2026-09-21/);
  assert.match(full, /Dated \*\*2026-09-21\*\* reaudit/);
  assert.match(wave5, /## GlossaryChip \(PASS\)/);
  assert.match(wave5, /PR 135 merge `a7facba`/);
  ...
  assert.match(campus, /PR 135 merge `a7facba`/);
  assert.match(campus, /GlossaryChip \*\*PASS\*\*/);
  assert.match(campus, /f4bf345d6c0ec8c51941b3bb2d08c71589bc822632d935dc26b6d18c07a631e7/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-glossary-chip\.md/);
  assert.match(src("AGENTS.md"), /GlossaryChipDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of [
    "GlossaryChipDemo-f0.png",
    "GlossaryChipDemo-f60.png",
...
test("checklist on section-encode master holds; --flip refused", () => {

...
const STILLS = "/opt/cursor/artifacts/remotion-antagonist-caveat-encode-master/2026-09-21";
const PRIOR =
...
test("full-set caveat encode master reaudit is dated PASS with hold_cleaning", () => {
  ...
  assert.match(full, /## LessonSpine caveat encode master dest reaudit 2026-09-21/);
  assert.match(full, /e23cadde85af2540d3c4f9c8a8434126655133b50c0cdc6c5783b16d9e446fd7/);
  ...
  assert.match(src("antagonist-caveat-encode-master.md"), /EDU-S01: PASS/);
  assert.match(src("antagonist-caveat-encode-master.md"), /EDU-S02: PASS/);
  assert.match(
    src("antagonist-caveat-encode-master.md"),
    /e23cadde85af2540d3c4f9c8a8434126655133b50c0cdc6c5783b16d9e446fd7/,
  );
  assert.match(
    src("antagonist-caveat-encode-master.md"),
    /PR 157 merge `5d39d6831f98628df7b0b4ac7e3c9e778cfb4a41`/,
  );
  assert.match(wave5, /## LessonSpine caveat encode master dest reaudit \(PASS\)/);
  assert.match(wave5, /PR 157 merge `5d39d68`/);
  ...
  assert.match(campus, /PR 157 merge `5d39d68`/);
  assert.match(campus, /LessonSpine caveat encode master dest reaudit \*\*PASS\*\*/);
  assert.match(campus, /e23cadde85af2540d3c4f9c8a8434126655133b50c0cdc6c5783b16d9e446fd7/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-caveat-encode-master\.md/);
  assert.doesNotMatch(wave5, /8\/8 PASS|launch OPEN/i);
});

test("locked caveat encode master dest and audit stills exist; priors untouched", () => {

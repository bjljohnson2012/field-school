...
const STILLS = "/opt/cursor/artifacts/remotion-quizbumper-next-up/2026-09-20";
const TYPECARD =
...
test("QuizBumper next-up kicker plus EDU-S02 Keyword gold tick", () => {
  ...
  assert.match(quiz, /import \{[^}]*Keyword[^}]*\} from "\.\/layers"/);
  assert.match(quiz, /<Kicker>Next up<\/Kicker>/);
  assert.match(quiz, /<Keyword>\{keyword\}<\/Keyword>/);
  assert.match(quiz, /words\.slice\(0, -1\)/);
  assert.doesNotMatch(quiz, /<Title>\{prompt\}<\/Title>/);
  assert.doesNotMatch(quiz, /Check yourself/);
  assert.match(quiz, /<LowerThird label="Quiz" \/>/);
  ...
  assert.doesNotMatch(quiz, /a_roll\.mp4|27pn9xs0zk8a73g/);
});

test("ORDER LOCK intact; docs seal QuizBumper next-up PASS with hold_cleaning", () => {
  ...
  const note = src("antagonist-quizbumper-next-up.md");
  ...
  assert.match(note, /PR 88 merge `253a638bb5d89847b2255b42e8d02ab8859f97ed`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up/,
  );
  assert.match(full, /## QuizBumper next-up 2026-09-20/);
  assert.match(full, /EDU-S02: PASS/);
  assert.match(full, /Dated \*\*2026-09-20\*\* reaudit/);
  assert.match(src("antagonist-definition-quiz-head.md"), /## QuizBumper next-up 2026-09-20/);
  assert.match(wave5, /## QuizBumper next-up \(PASS\)/);
  assert.match(wave5, /PR 88 merge `253a638`/);
  assert.match(wave5, /cab8bd9115d1782158bafeba5c36c820039bd05da3f4fc96412efe9932b86ebf/);
  assert.match(campus, /PR 88 merge `253a638`/);
  assert.match(campus, /QuizBumper next-up \*\*PASS\*\*/);
  assert.match(campus, /cab8bd9115d1782158bafeba5c36c820039bd05da3f4fc96412efe9932b86ebf/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-quizbumper-next-up\.md/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of ["QuizBumper-f54.png", "LessonSpine-nextup-f1074.png"]) {

...
const STILLS = "/opt/cursor/artifacts/remotion-antagonist-section-encode-master/2026-09-21";
const PRIOR =
...
test("full-set section encode master reaudit is dated PASS with hold_cleaning", () => {
  ...
  assert.match(full, /## LessonSpine section encode master dest reaudit 2026-09-21/);
  assert.match(full, /f4bf345d6c0ec8c51941b3bb2d08c71589bc822632d935dc26b6d18c07a631e7/);
  ...
  assert.match(src("antagonist-section-encode-master.md"), /EDU-S01: PASS/);
  assert.match(src("antagonist-section-encode-master.md"), /EDU-S02: PASS/);
  assert.match(
    src("antagonist-section-encode-master.md"),
    /f4bf345d6c0ec8c51941b3bb2d08c71589bc822632d935dc26b6d18c07a631e7/,
  );
  assert.match(
    src("antagonist-section-encode-master.md"),
    /PR 134 merge `03c531830a46db72ab577b1487a55207fdd2ff27`/,
  );
  assert.match(wave5, /## LessonSpine section encode master dest reaudit \(PASS\)/);
  assert.match(wave5, /PR 134 merge `03c5318`/);
  ...
  assert.match(campus, /PR 134 merge `03c5318`/);
  assert.match(campus, /LessonSpine section encode master dest reaudit \*\*PASS\*\*/);
  assert.match(campus, /f4bf345d6c0ec8c51941b3bb2d08c71589bc822632d935dc26b6d18c07a631e7/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-section-encode-master\.md/);
  assert.doesNotMatch(wave5, /8\/8 PASS|launch OPEN/i);
});

test("locked section encode master dest and audit stills exist; priors untouched", () => {

...
const MASTER_SHA = "244c485bbfd83e2138c4e499fc07a00f566be3d1ede1c5a3f03dfd57b04feb2d";
const STILLS = "/opt/cursor/artifacts/remotion-steps-card/2026-09-21";
const PRIOR =
  "/opt/cursor/artifacts/lesson-spine-example-encode/2026-09-21/LessonSpine.mp4";
const PRIOR_SHA = "f52ae59972fb4c0945febfa8b3166a217822548f8db6eff3468062fe07c0c33b";
...
test("StepsCard is a numbered procedure / do-this-in-order after sting/objective path", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "StepsCardDemo.tsx");
  const recap = src("src", "RecapCard.tsx");
  const board = src("src", "DefinitionBoard.tsx");
  assert.match(layers, /export function StepsCard/);
  assert.match(layers, /export const STEPS_CARD_KICKER/);
  assert.match(layers, /export const STEPS_CARD_LINE/);
  assert.match(layers, /STEPS_CARD_KICKER = "Steps"/);
  assert.match(layers, /STEPS_CARD_LINE = "Do this in order\."/);
  assert.match(layers, /STEPS_CARD_ONE = "1\. Name the beat\."/);
  assert.match(layers, /STEPS_CARD_TWO = "2\. Do the work\."/);
  assert.match(layers, /Numbered procedure \/ do-this-in-order/);
  assert.match(layers, /After sting\/objective path, after quote, before practice/);
  ...
  assert.match(demo, /StepsCardDemo/);
  assert.match(demo, /name="steps"/);
...
test("StepsCardDemo registers after QuoteCardDemo; Recap/Definition sequence steps after quote before practice", () => {
  ...
  assert.match(rootTsx, /id="StepsCardDemo"/);
  assert.match(rootTsx, /id="QuoteCardDemo"/);
  assert.match(rootTsx, /id="LessonSpine"/);
  assert.ok(rootTsx.indexOf('id="QuoteCardDemo"') < rootTsx.indexOf('id="StepsCardDemo"'));
  assert.ok(rootTsx.indexOf('id="StepsCardDemo"') < rootTsx.indexOf('id="LessonSpine"'));
  assert.doesNotMatch(opener, /StepsCard|name="steps"/);
  assert.doesNotMatch(head, /StepsCard|name="steps"/);
  assert.doesNotMatch(quiz, /StepsCard|name="steps"/);
  const boardWipe = board.indexOf('name="transition"');
  const boardQuote = board.indexOf('name="quote"');
  const boardSteps = board.indexOf('name="steps"');
  const boardAudio = board.indexOf('name="audio"');
  assert.ok(
    boardWipe >= 0 &&
      boardQuote > boardWipe &&
      boardSteps > boardQuote &&
...
test("ORDER LOCK intact; docs seal StepsCard PASS with hold_cleaning", () => {
  const note = src("antagonist-steps-card.md");
  ...
  assert.match(note, /244c485bbfd83e2138c4e499fc07a00f566be3d1ede1c5a3f03dfd57b04feb2d/);
  assert.match(note, /PR 152 merge `2a28cdaf19396988145960a0d43443ccca451f79`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## StepsCard 2026-09-21/);
  assert.match(full, /Dated \*\*2026-09-21\*\* reaudit/);
  assert.match(wave5, /## StepsCard \(PASS\)/);
  assert.match(wave5, /PR 152 merge `2a28cda`/);
  assert.match(wave5, /244c485bbfd83e2138c4e499fc07a00f566be3d1ede1c5a3f03dfd57b04feb2d/);
  assert.match(wave5, /\*\*Current master dest\.\*\*/);
  assert.match(campus, /PR 152 merge `2a28cda`/);
  assert.match(campus, /StepsCard \*\*PASS\*\*/);
  assert.match(campus, /244c485bbfd83e2138c4e499fc07a00f566be3d1ede1c5a3f03dfd57b04feb2d/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-steps-card\.md/);
  assert.match(src("AGENTS.md"), /StepsCardDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of [
    "StepsCardDemo-f0.png",
    "StepsCardDemo-f60.png",
...
test("checklist on quote-encode master holds; --flip refused", () => {

...
const MASTER_SHA = "8c953707401f835551ceec5d96c090b133cd619b41c4b7ff543b1637edc44dbf";
const STILLS = "/opt/cursor/artifacts/remotion-key-claim/2026-09-21";
const PRIOR =
  "/opt/cursor/artifacts/lesson-spine-end-card-encode/2026-09-20/LessonSpine.mp4";
const PRIOR_SHA = "073e1e3b18f3c6f2bc1590991d5bee4059c33487a19ff083c55927480d7a7693";
...
test("KeyClaim is a thesis / claim lock after sting/objective path", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "KeyClaimDemo.tsx");
  assert.match(layers, /export function KeyClaim/);
  assert.match(layers, /export const KEY_CLAIM_KICKER/);
  assert.match(layers, /export const KEY_CLAIM_LINE/);
  assert.match(layers, /KEY_CLAIM_KICKER = "Thesis"/);
  assert.match(layers, /KEY_CLAIM_LINE = "Lock one claim\."/);
  assert.match(layers, /Thesis \/ claim lock/);
  assert.match(layers, /After sting\/objective path, before practice/);
  ...
  assert.match(demo, /KeyClaimDemo/);
  assert.match(demo, /name="claim"/);
  assert.match(demo, /<KeyClaim \/>/);
  assert.match(demo, /beat="objective"/);
  assert.doesNotMatch(demo, /a_roll\.mp4|27pn9xs0zk8a73g/);
});

test("KeyClaimDemo registers after PracticeCardDemo; Opener/Definition/Recap sequence claim after sting/objective path", () => {
...
test("ORDER LOCK intact; docs seal KeyClaim PASS with hold_cleaning", () => {
  const note = src("antagonist-key-claim.md");
  ...
  assert.match(note, /8c953707401f835551ceec5d96c090b133cd619b41c4b7ff543b1637edc44dbf/);
  assert.match(note, /PR 123 merge `0b8a7fdc371aad35a4c03a6f8a84a621f7b1364e`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## KeyClaim 2026-09-21/);
  assert.match(full, /Dated \*\*2026-09-21\*\* reaudit/);
  assert.match(wave5, /## KeyClaim \(PASS\)/);
  assert.match(wave5, /PR 123 merge `0b8a7fd`/);
  assert.match(wave5, /8c953707401f835551ceec5d96c090b133cd619b41c4b7ff543b1637edc44dbf/);
  assert.match(wave5, /\*\*Current master dest\.\*\*/);
  assert.match(campus, /PR 123 merge `0b8a7fd`/);
  assert.match(campus, /KeyClaim \*\*PASS\*\*/);
  assert.match(campus, /8c953707401f835551ceec5d96c090b133cd619b41c4b7ff543b1637edc44dbf/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-key-claim\.md/);
  assert.match(src("AGENTS.md"), /KeyClaimDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of ["KeyClaimDemo-f0.png", "KeyClaimDemo-f60.png", "LessonSpine-objective-f570.png"]) {
    assert.equal(existsSync(join(STILLS, name)), true, name);
  }
});

test("checklist on practice-encode master holds; --flip refused", () => {

...
const MASTER_SHA = "2b2dab3e239656e122d4dda3199e294ec836351a83120efaefe2aa54e4b8c068";
const STILLS = "/opt/cursor/artifacts/remotion-source-chip/2026-09-21";
const PRIOR =
...
test("SourceChip is a citation chip distinct from quote / verse after timeline", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "SourceChipDemo.tsx");
  const recap = src("src", "RecapCard.tsx");
  const board = src("src", "DefinitionBoard.tsx");
  assert.match(layers, /export function SourceChip/);
  assert.match(layers, /export const SOURCE_CHIP_KICKER/);
  assert.match(layers, /export const SOURCE_CHIP_LINE/);
  assert.match(layers, /SOURCE_CHIP_KICKER = "Source"/);
  assert.match(layers, /SOURCE_CHIP_CITE = "Field"/);
  assert.match(layers, /SOURCE_CHIP_LINE = "Name the source\."/);
  assert.match(layers, /Citation chip distinct from quote \/ verse/);
  assert.match(layers, /After sting\/objective path, after timeline, before practice/);
  ...
  assert.match(demo, /SourceChipDemo/);
  assert.match(demo, /name="source"/);
...
test("SourceChipDemo registers after TimelineRailDemo; Recap/Definition sequence source after timeline before practice", () => {
  ...
  assert.ok(rootTsx.indexOf('id="TimelineRailDemo"') < rootTsx.indexOf('id="SourceChipDemo"'));
  assert.ok(rootTsx.indexOf('id="SourceChipDemo"') < rootTsx.indexOf('id="LessonSpine"'));
  assert.doesNotMatch(opener, /SourceChip|name="source"/);
  assert.doesNotMatch(head, /SourceChip|name="source"/);
  assert.doesNotMatch(quiz, /SourceChip|name="source"/);
  ...
  assert.ok(
    boardWipe >= 0 &&
      boardTimeline > boardWipe &&
      boardSource > boardTimeline &&
...
test("ORDER LOCK intact; docs seal SourceChip PASS with hold_cleaning", () => {
  const note = src("antagonist-source-chip.md");
  ...
  assert.match(note, /PR 164 merge `86a32f23f3a7ee00218d9b50a90fe916b01ff207`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## SourceChip 2026-09-21/);
  assert.match(full, /Dated \*\*2026-09-21\*\* reaudit/);
  assert.match(wave5, /## SourceChip \(PASS\)/);
  assert.match(wave5, /PR 164 merge `86a32f2`/);
  ...
  assert.match(campus, /PR 164 merge `86a32f2`/);
  assert.match(campus, /SourceChip \*\*PASS\*\*/);
  assert.match(campus, /2b2dab3e239656e122d4dda3199e294ec836351a83120efaefe2aa54e4b8c068/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-source-chip\.md/);
  assert.match(src("AGENTS.md"), /SourceChipDemo/);
...
test("checklist on timeline-encode master holds; --flip refused", () => {

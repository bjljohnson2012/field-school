...
const STILLS = "/opt/cursor/artifacts/remotion-objectiveslate-slate/2026-09-20";
const EDU_S02 =
...
test("ObjectiveSlate keeps EDU-S01 prefix plus EDU-S02 Keyword gold tick", () => {
  const layers = src("src", "layers.tsx");
  const opener = src("src", "Opener.tsx");
  const recap = src("src", "RecapCard.tsx");
  const slate = layers.slice(layers.indexOf("export function ObjectiveSlate"), layers.indexOf("export function Keyword"));
  const keyword = layers.slice(layers.indexOf("export function Keyword"));
  assert.match(layers, /export function ObjectiveSlate/);
  assert.match(layers, /You will be able to/);
  assert.match(slate, /<Keyword>\{keyword\}<\/Keyword>/);
  assert.match(slate, /words\.slice\(0, -1\)/);
  assert.match(keyword, /color: gold/);
  assert.match(keyword, /borderBottom: `3px solid \$\{gold\}`/);
  assert.match(opener, /<ObjectiveSlate objective=\{objective\}/);
  assert.match(recap, /<ObjectiveSlate objective=\{objective\}/);
  assert.match(src("src", "objectiveSlate.ts"), /export const EDU_S01_PREFIX = "You will be able to"/);
  assert.match(src("src", "objectiveSlate.ts"), /export const EDU_S01_OBJECTIVE = "draw one idea per beat"/);
  assert.match(src("src", "DefinitionBoard.tsx"), /<Keyword>\{keyword\}<\/Keyword>/);
...
test("ORDER LOCK intact; docs seal ObjectiveSlate slate PASS with hold_cleaning", () => {
  ...
  const note = src("antagonist-objectiveslate-slate.md");
  ...
  assert.match(note, /PR 96 merge `c4aa0a44d82602f47d64d8b28b0bf1bf07ff2780`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up/,
  );
  assert.match(full, /## ObjectiveSlate slate 2026-09-20/);
  assert.match(full, /EDU-S02: PASS/);
  assert.match(full, /Dated \*\*2026-09-20\*\* reaudit/);
  assert.match(src("antagonist-opener-recap.md"), /## ObjectiveSlate slate 2026-09-20/);
  assert.match(wave5, /## ObjectiveSlate slate \(PASS\)/);
  assert.match(wave5, /PR 96 merge `c4aa0a4`/);
  ...
  assert.match(campus, /PR 96 merge `c4aa0a4`/);
  assert.match(campus, /ObjectiveSlate slate \*\*PASS\*\*/);
  assert.match(campus, /d0a09896aa1ea6fc8cf5b5898bc83ddb54faccaa0957c8529a3de0d49a10efc5/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-objectiveslate-slate\.md/);

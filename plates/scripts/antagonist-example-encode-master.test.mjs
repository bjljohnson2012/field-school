...
const STILLS = "/opt/cursor/artifacts/remotion-antagonist-example-encode-master/2026-09-21";
const PRIOR =
...
test("full-set example encode master reaudit is dated PASS with hold_cleaning", () => {
  ...
  assert.match(full, /## LessonSpine example encode master dest reaudit 2026-09-21/);
  assert.match(full, /f52ae59972fb4c0945febfa8b3166a217822548f8db6eff3468062fe07c0c33b/);
  ...
  assert.match(src("antagonist-example-encode-master.md"), /EDU-S01: PASS/);
  assert.match(src("antagonist-example-encode-master.md"), /EDU-S02: PASS/);
  assert.match(
    src("antagonist-example-encode-master.md"),
    /f52ae59972fb4c0945febfa8b3166a217822548f8db6eff3468062fe07c0c33b/,
  );
  assert.match(
    src("antagonist-example-encode-master.md"),
    /PR 148 merge `86b223aaab708e117359a6f2c3f670e45f5a3689`/,
  );
  assert.match(wave5, /## LessonSpine example encode master dest reaudit \(PASS\)/);
  assert.match(wave5, /PR 148 merge `86b223a`/);
  ...
  assert.match(campus, /PR 148 merge `86b223a`/);
  assert.match(campus, /LessonSpine example encode master dest reaudit \*\*PASS\*\*/);
  assert.match(campus, /f52ae59972fb4c0945febfa8b3166a217822548f8db6eff3468062fe07c0c33b/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-example-encode-master\.md/);
  assert.doesNotMatch(wave5, /8\/8 PASS|launch OPEN/i);
});

test("locked example encode master dest and audit stills exist; priors untouched", () => {

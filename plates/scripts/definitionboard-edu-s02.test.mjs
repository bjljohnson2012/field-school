...
const STILLS = "/opt/cursor/artifacts/remotion-definitionboard-edu-s02/2026-09-20";
const TYPECARD =
...
test("DefinitionBoard Title last word is EDU-S02 Keyword gold tick", () => {
...
test("ORDER LOCK intact; docs seal EDU-S02 PASS with hold_cleaning", () => {
  ...
  const note = src("antagonist-definitionboard-edu-s02.md");
  ...
  assert.match(note, /PR 87 merge `c2e9dad0d395eb6d3ca5a0cf7166a92c57dff109`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up/,
  );
  assert.match(full, /## EDU-S02 DefinitionBoard 2026-09-20/);
  assert.match(full, /EDU-S02: PASS/);
  assert.match(full, /Dated \*\*2026-09-20\*\* reaudit/);
  assert.match(src("antagonist-definition-quiz-head.md"), /## EDU-S02 DefinitionBoard 2026-09-20/);
  assert.match(wave5, /## EDU-S02 DefinitionBoard \(PASS\)/);
  assert.match(wave5, /PR 87 merge `c2e9dad`/);
  assert.match(wave5, /cab8bd9115d1782158bafeba5c36c820039bd05da3f4fc96412efe9932b86ebf/);
  assert.match(campus, /PR 87 merge `c2e9dad`/);
  assert.match(campus, /EDU-S02 DefinitionBoard \*\*PASS\*\*/);
  assert.match(campus, /cab8bd9115d1782158bafeba5c36c820039bd05da3f4fc96412efe9932b86ebf/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-definitionboard-edu-s02\.md/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of ["DefinitionBoard-f30.png", "LessonSpine-objective-f570.png"]) {

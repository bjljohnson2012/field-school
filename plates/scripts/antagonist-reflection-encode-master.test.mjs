...
const STILLS = "/opt/cursor/artifacts/remotion-antagonist-reflection-encode-master/2026-09-21";
const PRIOR =
...
test("full-set reflection encode master reaudit is dated PASS with hold_cleaning", () => {
  ...
  assert.match(full, /## LessonSpine reflection encode master dest reaudit 2026-09-21/);
  assert.match(full, /1e244f82eabceff7e3b16f39d41ff8d49e002aec188c694257f5da9bb4e77f17/);
  ...
  assert.match(src("antagonist-reflection-encode-master.md"), /EDU-S01: PASS/);
  assert.match(src("antagonist-reflection-encode-master.md"), /EDU-S02: PASS/);
  assert.match(
    src("antagonist-reflection-encode-master.md"),
    /1e244f82eabceff7e3b16f39d41ff8d49e002aec188c694257f5da9bb4e77f17/,
  );
  assert.match(
    src("antagonist-reflection-encode-master.md"),
    /PR 160 merge `907a79973420288bf6ece9315fd1d87d6b45b6a4`/,
  );
  assert.match(wave5, /## LessonSpine reflection encode master dest reaudit \(PASS\)/);
  assert.match(wave5, /PR 160 merge `907a799`/);
  ...
  assert.match(campus, /PR 160 merge `907a799`/);
  assert.match(campus, /LessonSpine reflection encode master dest reaudit \*\*PASS\*\*/);
  assert.match(campus, /1e244f82eabceff7e3b16f39d41ff8d49e002aec188c694257f5da9bb4e77f17/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-reflection-encode-master\.md/);
  assert.doesNotMatch(wave5, /8\/8 PASS|launch OPEN/i);
});

test("locked reflection encode master dest and audit stills exist; priors untouched", () => {

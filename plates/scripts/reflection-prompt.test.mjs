...
const MASTER_SHA = "e23cadde85af2540d3c4f9c8a8434126655133b50c0cdc6c5783b16d9e446fd7";
const STILLS = "/opt/cursor/artifacts/remotion-reflection-prompt/2026-09-21";
const PRIOR =
  "/opt/cursor/artifacts/lesson-spine-steps-encode/2026-09-21/LessonSpine.mp4";
const PRIOR_SHA = "e0ead459d9505cd612403e8601ab1bfd9ef1acbfc3eba59d3640e19ce621935b";
...
test("ReflectionPrompt is a learner pause / think-then-answer after caveat", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "ReflectionPromptDemo.tsx");
  const recap = src("src", "RecapCard.tsx");
  const board = src("src", "DefinitionBoard.tsx");
  assert.match(layers, /export function ReflectionPrompt/);
  assert.match(layers, /export const REFLECTION_PROMPT_KICKER/);
  assert.match(layers, /export const REFLECTION_PROMPT_LINE/);
  assert.match(layers, /REFLECTION_PROMPT_KICKER = "Reflect"/);
  assert.match(layers, /REFLECTION_PROMPT_LINE = "Think, then answer\."/);
  assert.match(layers, /Learner pause \/ think-then-answer/);
  assert.match(layers, /After sting\/objective path, after caveat, before practice/);
  ...
  assert.match(demo, /ReflectionPromptDemo/);
  assert.match(demo, /name="reflect"/);
...
test("ReflectionPromptDemo registers after CaveatCardDemo; Recap/Definition sequence reflect after caveat before practice", () => {
  ...
  assert.match(rootTsx, /id="ReflectionPromptDemo"/);
  assert.match(rootTsx, /id="CaveatCardDemo"/);
  assert.match(rootTsx, /id="LessonSpine"/);
  assert.ok(rootTsx.indexOf('id="CaveatCardDemo"') < rootTsx.indexOf('id="ReflectionPromptDemo"'));
  assert.ok(rootTsx.indexOf('id="ReflectionPromptDemo"') < rootTsx.indexOf('id="LessonSpine"'));
  assert.doesNotMatch(opener, /ReflectionPrompt|name="reflect"/);
  assert.doesNotMatch(head, /ReflectionPrompt|name="reflect"/);
  assert.doesNotMatch(quiz, /ReflectionPrompt|name="reflect"/);
  const boardWipe = board.indexOf('name="transition"');
  const boardCaveat = board.indexOf('name="caveat"');
  const boardReflect = board.indexOf('name="reflect"');
  const boardAudio = board.indexOf('name="audio"');
  assert.ok(
    boardWipe >= 0 &&
      boardCaveat > boardWipe &&
      boardReflect > boardCaveat &&
...
test("ORDER LOCK intact; docs seal ReflectionPrompt PASS with hold_cleaning", () => {
  const note = src("antagonist-reflection-prompt.md");
  ...
  assert.match(note, /e23cadde85af2540d3c4f9c8a8434126655133b50c0cdc6c5783b16d9e446fd7/);
  assert.match(note, /PR 158 merge `2a90b5a05df9e441e72131fc39ab055caeb6011d`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## ReflectionPrompt 2026-09-21/);
  assert.match(full, /Dated \*\*2026-09-21\*\* reaudit/);
  assert.match(wave5, /## ReflectionPrompt \(PASS\)/);
  assert.match(wave5, /PR 158 merge `2a90b5a`/);
  assert.match(wave5, /e23cadde85af2540d3c4f9c8a8434126655133b50c0cdc6c5783b16d9e446fd7/);
  assert.match(wave5, /\*\*Current master dest\.\*\*/);
  assert.match(campus, /PR 158 merge `2a90b5a`/);
  assert.match(campus, /ReflectionPrompt \*\*PASS\*\*/);
  assert.match(campus, /e23cadde85af2540d3c4f9c8a8434126655133b50c0cdc6c5783b16d9e446fd7/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-reflection-prompt\.md/);
  assert.match(src("AGENTS.md"), /ReflectionPromptDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of [
    "ReflectionPromptDemo-f0.png",
    "ReflectionPromptDemo-f60.png",
...
test("checklist on caveat-encode master holds; --flip refused", () => {

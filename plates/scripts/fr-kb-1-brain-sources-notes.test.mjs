...
function writeSourceAndNote(childId, name, sourceBody, noteBody) {
  ...
  assert.equal(started.selected?.kind, "child");
  assert.equal(started.selected?.login, "none");
  assert.equal(started.selected?.user, false);
  assert.ok((started.selected?.sources || []).length >= 1);
  assert.ok((started.selected?.notes || []).length >= 1);
  ...
  assert.equal(selected.selected?.sources[0]?.body, sourceBody);
  assert.equal(selected.selected?.notes[0]?.body, noteBody);
  assert.ok((selected.selected?.summary.sources || 0) >= 1);
  assert.ok((selected.selected?.summary.notes || 0) >= 1);
...
test("FR-KB-1 sources and notes do not steal family LIVE or family brain store", () => {
  ...
  assert.match(ui, /data-brain-sources/);
  assert.match(ui, /data-brain-notes/);
  assert.match(ui, /data-brain-source-count/);
  assert.match(ui, /data-brain-note-count/);
  assert.match(api, /sources: body.sources/);
  assert.match(api, /notes: body.notes/);
...
test("master and priors untouched after sources and notes", () => {
...
test("WAVE5 STATUS cite Sources and notes after PR 199 merge; launch CLOSED 0/8", () => {
  ...
  assert.match(wave5, /## Sources and notes \(PASS\)/);
  assert.match(wave5, /PR 199 merge `0a10678`/);
  ...
  assert.match(campus, /PR 199 merge `0a10678`/);
  ...
  assert.match(rail, /Sources and notes/);
  assert.match(rail, /\/api\/progress\/brain/);
  assert.match(gate, /Product \| HELD/);
  assert.match(gate, /Sources and notes FR-KB-1 \/ FR-KB-2/);

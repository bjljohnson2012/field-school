...
const STILLS = "/opt/cursor/artifacts/remotion-caption-density/2026-09-20";
const PRIOR =
...
test("CaptionDensity packs karaoke into six-word lines", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "CaptionDensityDemo.tsx");
  assert.match(layers, /export const CAPTION_DENSITY_WORDS/);
  assert.match(layers, /export function packCaptionLines/);
  assert.match(layers, /export function packedCaptionLine/);
  assert.match(layers, /Karaoke line packing/);
  assert.match(layers, /Caption density/);
  assert.match(layers, /CAPTION_DENSITY_WORDS = 6/);
  assert.match(layers, /packedCaptionLine\(clock\.words\)/);
  ...
  assert.match(demo, /CaptionDensityDemo/);
...
test("CaptionDensityDemo registers after CaptionsDemo; ORDER LOCK intact", () => {
  ...
  assert.ok(rootTsx.indexOf('id="CaptionsDemo"') < rootTsx.indexOf('id="CaptionDensityDemo"'));
...
test("ORDER LOCK intact; docs seal CaptionDensity PASS with hold_cleaning", () => {
  const note = src("antagonist-caption-density.md");
  ...
  assert.match(note, /PR 110 merge `380b36d41922951e04e0d96f1400dff1ced036e3`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up/,
  );
  assert.match(full, /## CaptionDensity 2026-09-20/);
  assert.match(full, /Dated \*\*2026-09-20\*\* reaudit/);
  assert.match(wave5, /## CaptionDensity \(PASS\)/);
  assert.match(wave5, /PR 110 merge `380b36d`/);
  ...
  assert.match(campus, /PR 110 merge `380b36d`/);
  assert.match(campus, /CaptionDensity \*\*PASS\*\*/);
  assert.match(campus, /68b8378a47d9eb57ed4b41f35bc2bf4ed6fd01a0694fca28c0a5ea90101fee92/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-caption-density\.md/);
  assert.match(src("AGENTS.md"), /CaptionDensityDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of ["CaptionDensityDemo-f0.png", "CaptionDensityDemo-f60.png", "LessonSpine-sting-f42.png"]) {

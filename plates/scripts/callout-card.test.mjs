...
const STILLS = "/opt/cursor/artifacts/remotion-callout-card/2026-09-20";
const PRIOR =
...
test("CalloutCard is a tip / aside signal after chapter", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "CalloutCardDemo.tsx");
  assert.match(layers, /export function CalloutCard/);
  assert.match(layers, /export const CALLOUT_CARD_COPY/);
  assert.match(layers, /kicker: "Tip"/);
  assert.match(layers, /kicker: "Aside"/);
  assert.match(layers, /One idea per beat/);
  assert.match(layers, /Head stays docked/);
  assert.match(layers, /Type is the lesson/);
  assert.match(layers, /Return the same claim/);
  assert.match(layers, /Next beat, not a dump/);
  assert.match(layers, /right: 64/);
  assert.match(layers, /bottom: LETTERBOX_H \+ 28/);
  assert.match(layers, /Tip \/ aside signal/);
  assert.match(layers, /After chapter, before audio/);
  ...
  assert.match(demo, /CalloutCardDemo/);
  assert.match(demo, /name="callout"/);
  assert.match(demo, /<CalloutCard beat=\{beat\} \/>/);
  assert.match(demo, /Math\.floor\(frame \/ 48\)/);
  assert.doesNotMatch(demo, /a_roll\.mp4|27pn9xs0zk8a73g/);
});

test("CalloutCardDemo registers; five spine plates carry callout between chapter and audio", () => {
  ...
  for (const [file, beat] of Object.entries(plates)) {
    ...
    const chapter = blob.indexOf('name="chapter"');
    const callout = blob.indexOf('name="callout"');
    const audio = blob.indexOf('name="audio"');
    assert.ok(chapter >= 0 && callout > chapter && audio > callout, file);
    assert.match(blob, new RegExp(`<CalloutCard beat="${beat}" />`));
    assert.match(blob, new RegExp(`<ChapterChip beat="${beat}" />`));
...
test("ORDER LOCK intact; docs seal CalloutCard PASS with hold_cleaning", () => {
  const note = src("antagonist-callout-card.md");
  ...
  assert.match(note, /PR 107 merge `a8585c203b3186d1434430e686e27c8f7a8b2c8a`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up/,
  );
  assert.match(full, /## CalloutCard 2026-09-20/);
  assert.match(full, /Dated \*\*2026-09-20\*\* reaudit/);
  assert.match(wave5, /## CalloutCard \(PASS\)/);
  assert.match(wave5, /PR 107 merge `a8585c`/);
  ...
  assert.match(campus, /PR 107 merge `a8585c`/);
  assert.match(campus, /CalloutCard \*\*PASS\*\*/);
  assert.match(campus, /a11e31a38774266b80a8f64e088ff4eb0b59d80bc7b710e9240b6ee076e61ce7/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-callout-card\.md/);
  assert.match(src("AGENTS.md"), /CalloutCardDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of ["CalloutCardDemo-f0.png", "CalloutCardDemo-f60.png", "LessonSpine-sting-f42.png"]) {

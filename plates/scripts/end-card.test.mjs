...
const MASTER_SHA = "f631402a6dfbb2ca2218ea9617c7b2af527eb7e88f8df5547dc6584c9ef5a96e";
const STILLS = "/opt/cursor/artifacts/remotion-end-card/2026-09-20";
const PRIOR =
  "/opt/cursor/artifacts/lesson-spine-caption-overlay-encode/2026-09-20/LessonSpine.mp4";
const PRIOR_SHA = "5b229fee3254426ba8b6dca0c7af23f2270b485b7b62f296b5ae6a31b4d5c61c";
...
test("EndCard is a lesson close signal after next-up", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "EndCardDemo.tsx");
  assert.match(layers, /export function EndCard/);
  assert.match(layers, /export const END_CARD_KICKER/);
  assert.match(layers, /export const END_CARD_LINE/);
  assert.match(layers, /END_CARD_KICKER = "Close"/);
  assert.match(layers, /END_CARD_LINE = "The lesson holds\."/);
  assert.match(layers, /Lesson close signal/);
  assert.match(layers, /After QuizBumper\/next-up, before audio/);
  ...
  assert.match(demo, /EndCardDemo/);
  assert.match(demo, /name="end"/);
  assert.match(demo, /<EndCard \/>/);
  assert.match(demo, /beat="next-up"/);
  assert.doesNotMatch(demo, /a_roll\.mp4|27pn9xs0zk8a73g/);
});

test("EndCardDemo registers after TransitionLumaDemo; QuizBumper sequences EndCard after next-up", () => {
...
test("ORDER LOCK intact; docs seal EndCard PASS with hold_cleaning", () => {
  const note = src("antagonist-end-card.md");
  ...
  assert.match(note, /f631402a6dfbb2ca2218ea9617c7b2af527eb7e88f8df5547dc6584c9ef5a96e/);
  assert.match(note, /PR 117 merge `37d7fded486f027974599daabe71ebb7ce7c126f`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## EndCard 2026-09-20/);
  assert.match(full, /Dated \*\*2026-09-20\*\* reaudit/);
  assert.match(wave5, /## EndCard \(PASS\)/);
  assert.match(wave5, /PR 117 merge `37d7fde`/);
  ...
  assert.match(campus, /PR 117 merge `37d7fde`/);
  assert.match(campus, /EndCard \*\*PASS\*\*/);
  assert.match(campus, /f631402a6dfbb2ca2218ea9617c7b2af527eb7e88f8df5547dc6584c9ef5a96e/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-end-card\.md/);
  assert.match(src("AGENTS.md"), /EndCardDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of ["EndCardDemo-f0.png", "EndCardDemo-f60.png", "LessonSpine-nextup-f1050.png"]) {
    assert.equal(existsSync(join(STILLS, name)), true, name);
  }
});

test("checklist on transition-luma master holds; --flip refused", () => {

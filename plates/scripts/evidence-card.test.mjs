...
const MASTER_SHA = "4aef4c713218247ebb4cad42b637f4b1331a02d6db4fcc13ac13e2bde156a337";
const STILLS = "/opt/cursor/artifacts/remotion-evidence-card/2026-09-21";
const PRIOR =
  "/opt/cursor/artifacts/lesson-spine-threshold-encode/2026-09-21/LessonSpine.mp4";
const PRIOR_SHA = "955f0256424fdc4ddb5ae5506a04a9ea877cb6a79ccb126ba1bb4884b9943a6f";

const PRIORS = {
  [PRIOR]: PRIOR_SHA,
  "/opt/cursor/artifacts/lesson-spine-spectrum-encode/2026-09-21/LessonSpine.mp4":
    "496f506babd11a6e5247e87c3bbf926117b0de31feae5b6573fa891bdf6c68d0",
...
test("EvidenceCard is evidence / warrant rows distinct from bands and citation after rubric", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "EvidenceCardDemo.tsx");
  const recap = src("src", "RecapCard.tsx");
  const board = src("src", "DefinitionBoard.tsx");
  assert.match(layers, /export function EvidenceCard/);
  assert.match(layers, /export const EVIDENCE_CARD_KICKER/);
  assert.match(layers, /export const EVIDENCE_CARD_LINE/);
  assert.match(layers, /EVIDENCE_CARD_KICKER = "Evidence"/);
  assert.match(layers, /EVIDENCE_CARD_LINE = "Hold the warrant\."/);
  assert.match(layers, /EVIDENCE_CARD_ONE = "The fact holds\."/);
  assert.match(layers, /EVIDENCE_CARD_TWO = "The why shows\."/);
  assert.match(layers, /Evidence \/ warrant rows distinct from bands and citation/);
  assert.match(layers, /After sting\/objective path, after rubric, before practice/);
  ...
  assert.match(demo, /EvidenceCardDemo/);
  assert.match(demo, /name="evidence"/);
...
test("EvidenceCardDemo registers after RubricCardDemo; Recap/Definition sequence evidence after rubric before practice", () => {
  ...
  assert.match(rootTsx, /id="EvidenceCardDemo"/);
  assert.match(rootTsx, /id="RubricCardDemo"/);
  assert.match(rootTsx, /id="LessonSpine"/);
  assert.ok(rootTsx.indexOf('id="RubricCardDemo"') < rootTsx.indexOf('id="EvidenceCardDemo"'));
  assert.ok(rootTsx.indexOf('id="EvidenceCardDemo"') < rootTsx.indexOf('id="LessonSpine"'));
  assert.doesNotMatch(opener, /EvidenceCard|name="evidence"/);
  assert.doesNotMatch(head, /EvidenceCard|name="evidence"/);
  assert.doesNotMatch(quiz, /EvidenceCard|name="evidence"/);
  const boardWipe = board.indexOf('name="transition"');
  const boardRubric = board.indexOf('name="rubric"');
  const boardEvidence = board.indexOf('name="evidence"');
  const boardAudio = board.indexOf('name="audio"');
  assert.ok(
    boardWipe >= 0 &&
      boardRubric > boardWipe &&
      boardEvidence > boardRubric &&
...
test("ORDER LOCK intact; docs seal EvidenceCard PASS with hold_cleaning", () => {
  const note = src("antagonist-evidence-card.md");
  ...
  assert.match(note, /4aef4c713218247ebb4cad42b637f4b1331a02d6db4fcc13ac13e2bde156a337/);
  assert.match(note, /PR 176 merge `9e92f64e8b79a0dfb81ead8c5152481d19e6bbd3`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## EvidenceCard 2026-09-21/);
  assert.match(full, /Dated \*\*2026-09-21\*\* reaudit/);
  assert.match(wave5, /## EvidenceCard \(PASS\)/);
  assert.match(wave5, /PR 176 merge `9e92f64`/);
  ...
  assert.match(campus, /PR 176 merge `9e92f64`/);
  assert.match(campus, /EvidenceCard \*\*PASS\*\*/);
  assert.match(campus, /4aef4c713218247ebb4cad42b637f4b1331a02d6db4fcc13ac13e2bde156a337/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-evidence-card\.md/);
  assert.match(src("AGENTS.md"), /EvidenceCardDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of [
    "EvidenceCardDemo-f0.png",
    "EvidenceCardDemo-f60.png",
...
test("checklist on rubric-encode master holds; --flip refused", () => {

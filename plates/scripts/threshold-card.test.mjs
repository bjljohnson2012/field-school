...
const MASTER_SHA = "496f506babd11a6e5247e87c3bbf926117b0de31feae5b6573fa891bdf6c68d0";
const STILLS = "/opt/cursor/artifacts/remotion-threshold-card/2026-09-21";
const PRIOR =
  "/opt/cursor/artifacts/lesson-spine-source-encode/2026-09-21/LessonSpine.mp4";
const PRIOR_SHA = "833d39f430b657e09534b5ddd011e321ce2697f590738001c4c217af6aef1478";
...
test("ThresholdCard is a cutoff / gate mark distinct from continuum after spectrum", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "ThresholdCardDemo.tsx");
  const recap = src("src", "RecapCard.tsx");
  const board = src("src", "DefinitionBoard.tsx");
  assert.match(layers, /export function ThresholdCard/);
  assert.match(layers, /export const THRESHOLD_CARD_KICKER/);
  assert.match(layers, /export const THRESHOLD_CARD_LINE/);
  assert.match(layers, /THRESHOLD_CARD_KICKER = "Threshold"/);
  assert.match(layers, /THRESHOLD_CARD_LINE = "Mark the cutoff\."/);
  assert.match(layers, /THRESHOLD_CARD_MARK = "Here\."/);
  assert.match(layers, /Cutoff \/ gate mark distinct from continuum/);
  assert.match(layers, /After sting\/objective path, after spectrum, before practice/);
  assert.match(layers, /borderLeft: `6px solid \$\{gold\}`/);
  assert.match(layers, /<Keyword>\{keyword\}<\/Keyword>/);
  assert.match(layers, /<Keyword>\{THRESHOLD_CARD_MARK\}<\/Keyword>/);
  ...
  assert.match(demo, /ThresholdCardDemo/);
  assert.match(demo, /name="threshold"/);
...
test("ThresholdCardDemo registers after SpectrumBarDemo; Recap/Definition sequence threshold after spectrum before practice", () => {
  ...
  assert.ok(rootTsx.indexOf('id="SpectrumBarDemo"') < rootTsx.indexOf('id="ThresholdCardDemo"'));
  assert.ok(rootTsx.indexOf('id="ThresholdCardDemo"') < rootTsx.indexOf('id="LessonSpine"'));
  assert.doesNotMatch(opener, /ThresholdCard|name="threshold"/);
  assert.doesNotMatch(head, /ThresholdCard|name="threshold"/);
  assert.doesNotMatch(quiz, /ThresholdCard|name="threshold"/);
  ...
  assert.ok(
    boardWipe >= 0 &&
      boardSpectrum > boardWipe &&
      boardThreshold > boardSpectrum &&
...
test("ORDER LOCK intact; docs seal ThresholdCard PASS with hold_cleaning", () => {
  const note = src("antagonist-threshold-card.md");
  ...
  assert.match(note, /496f506babd11a6e5247e87c3bbf926117b0de31feae5b6573fa891bdf6c68d0/);
  assert.match(note, /PR 170 merge `cad987229703b8ce6561ac7731f969faf233cd97`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## ThresholdCard 2026-09-21/);
  assert.match(full, /Dated \*\*2026-09-21\*\* reaudit/);
  assert.match(wave5, /## ThresholdCard \(PASS\)/);
  assert.match(wave5, /PR 170 merge `cad9872`/);
  assert.match(wave5, /496f506babd11a6e5247e87c3bbf926117b0de31feae5b6573fa891bdf6c68d0/);
  assert.match(wave5, /\*\*Current master dest\.\*\*/);
  assert.match(campus, /PR 170 merge `cad9872`/);
  assert.match(campus, /ThresholdCard \*\*PASS\*\*/);
  assert.match(campus, /496f506babd11a6e5247e87c3bbf926117b0de31feae5b6573fa891bdf6c68d0/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-threshold-card\.md/);
  assert.match(src("AGENTS.md"), /ThresholdCardDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of [
    "ThresholdCardDemo-f0.png",
    "ThresholdCardDemo-f60.png",
...
test("checklist on spectrum-encode master holds; --flip refused", () => {

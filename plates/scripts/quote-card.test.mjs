...
const MASTER_SHA = "f52ae59972fb4c0945febfa8b3166a217822548f8db6eff3468062fe07c0c33b";
const STILLS = "/opt/cursor/artifacts/remotion-quote-card/2026-09-21";
const PRIOR =
  "/opt/cursor/artifacts/lesson-spine-checkpoint-encode/2026-09-21/LessonSpine.mp4";
const PRIOR_SHA = "19e5a19c47f72197bbf726707b839bdf287fd3f0111d105b2161fd694584246a";
...
test("QuoteCard is a pull quote / authority cite after sting/objective path", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "QuoteCardDemo.tsx");
  const recap = src("src", "RecapCard.tsx");
  const board = src("src", "DefinitionBoard.tsx");
  assert.match(layers, /export function QuoteCard/);
  assert.match(layers, /export const QUOTE_CARD_KICKER/);
  assert.match(layers, /export const QUOTE_CARD_LINE/);
  assert.match(layers, /QUOTE_CARD_KICKER = "Quote"/);
  assert.match(layers, /QUOTE_CARD_LINE = "Hold the quote\."/);
  assert.match(layers, /Pull quote \/ authority cite/);
  assert.match(layers, /After sting\/objective path, after example, before practice/);
  ...
  assert.match(demo, /QuoteCardDemo/);
  assert.match(demo, /name="quote"/);
...
test("QuoteCardDemo registers after ExampleCardDemo; Recap/Definition sequence quote after example before practice", () => {
  ...
  assert.ok(rootTsx.indexOf('id="ExampleCardDemo"') < rootTsx.indexOf('id="QuoteCardDemo"'));
  assert.ok(rootTsx.indexOf('id="QuoteCardDemo"') < rootTsx.indexOf('id="LessonSpine"'));
  assert.doesNotMatch(opener, /QuoteCard|name="quote"/);
  assert.doesNotMatch(head, /QuoteCard|name="quote"/);
  assert.doesNotMatch(quiz, /QuoteCard|name="quote"/);
  ...
  assert.ok(
    boardWipe >= 0 &&
      boardExample > boardWipe &&
      boardQuote > boardExample &&
...
test("ORDER LOCK intact; docs seal QuoteCard PASS with hold_cleaning", () => {
  const note = src("antagonist-quote-card.md");
  ...
  assert.match(note, /f52ae59972fb4c0945febfa8b3166a217822548f8db6eff3468062fe07c0c33b/);
  assert.match(note, /PR 149 merge `f0514c1557ed965709c90044b7e8915d8ae90ed1`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## QuoteCard 2026-09-21/);
  assert.match(full, /Dated \*\*2026-09-21\*\* reaudit/);
  assert.match(wave5, /## QuoteCard \(PASS\)/);
  assert.match(wave5, /PR 149 merge `f0514c1`/);
  assert.match(wave5, /f52ae59972fb4c0945febfa8b3166a217822548f8db6eff3468062fe07c0c33b/);
  assert.match(wave5, /\*\*Current master dest\.\*\*/);
  assert.match(campus, /PR 149 merge `f0514c1`/);
  assert.match(campus, /QuoteCard \*\*PASS\*\*/);
  assert.match(campus, /f52ae59972fb4c0945febfa8b3166a217822548f8db6eff3468062fe07c0c33b/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-quote-card\.md/);
  assert.match(src("AGENTS.md"), /QuoteCardDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of [
    "QuoteCardDemo-f0.png",
    "QuoteCardDemo-f60.png",
...
test("checklist on example-encode master holds; --flip refused", () => {

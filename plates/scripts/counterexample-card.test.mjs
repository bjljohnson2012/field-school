...
const STILLS = "/opt/cursor/artifacts/remotion-counterexample-card/2026-09-21";
const PRIOR =
...
const PRIORS = {
  ...
  "/opt/cursor/artifacts/lesson-spine-rubric-encode/2026-09-21/LessonSpine.mp4":
    "4aef4c713218247ebb4cad42b637f4b1331a02d6db4fcc13ac13e2bde156a337",
  "/opt/cursor/artifacts/lesson-spine-threshold-encode/2026-09-21/LessonSpine.mp4":
    "955f0256424fdc4ddb5ae5506a04a9ea877cb6a79ccb126ba1bb4884b9943a6f",
...
test("CounterexampleCard is counterexample / not-this foil distinct from show-the-work and like-this transfer after analogy", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "CounterexampleCardDemo.tsx");
  const recap = src("src", "RecapCard.tsx");
  const board = src("src", "DefinitionBoard.tsx");
  assert.match(layers, /export function CounterexampleCard/);
  assert.match(layers, /export const COUNTEREXAMPLE_CARD_KICKER/);
  assert.match(layers, /export const COUNTEREXAMPLE_CARD_LINE/);
  assert.match(layers, /COUNTEREXAMPLE_CARD_KICKER = "Foil"/);
  assert.match(layers, /COUNTEREXAMPLE_CARD_LINE = "Name what is not\."/);
  assert.match(layers, /COUNTEREXAMPLE_CARD_ONE = "Not this case\."/);
  assert.match(layers, /COUNTEREXAMPLE_CARD_TWO = "A miss\."/);
  assert.match(layers, /Counterexample \/ not-this foil distinct from show-the-work and like-this transfer/);
  assert.match(layers, /After sting\/objective path, after analogy, before practice/);
  ...
  assert.match(layers, /borderRadius: 16/);
  ...
  assert.match(demo, /CounterexampleCardDemo/);
  assert.match(demo, /name="counterexample"/);
...
test("CounterexampleCardDemo registers after AnalogyCardDemo; Recap/Definition sequence counterexample after analogy before practice", () => {
  ...
  assert.match(rootTsx, /id="AnalogyCardDemo"/);
  assert.match(rootTsx, /id="LessonSpine"/);
  assert.ok(rootTsx.indexOf('id="AnalogyCardDemo"') < rootTsx.indexOf('id="CounterexampleCardDemo"'));
  assert.ok(rootTsx.indexOf('id="CounterexampleCardDemo"') < rootTsx.indexOf('id="LessonSpine"'));
  assert.doesNotMatch(opener, /CounterexampleCard|name="counterexample"/);
  assert.doesNotMatch(head, /CounterexampleCard|name="counterexample"/);
  assert.doesNotMatch(quiz, /CounterexampleCard|name="counterexample"/);
  const boardWipe = board.indexOf('name="transition"');
  const boardAnalogy = board.indexOf('name="analogy"');
  const boardCounter = board.indexOf('name="counterexample"');
  const boardAudio = board.indexOf('name="audio"');
  assert.ok(
    boardWipe >= 0 &&
      boardAnalogy > boardWipe &&
      boardCounter > boardAnalogy &&
...
test("ORDER LOCK intact; docs seal CounterexampleCard PASS with hold_cleaning", () => {
  const note = src("antagonist-counterexample-card.md");
  ...
  assert.match(note, /PR 182 merge `a464568e7a432ff6d03852170637515dbd4ac952`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## CounterexampleCard 2026-09-21/);
  assert.match(full, /Dated \*\*2026-09-21\*\* reaudit/);
  assert.match(wave5, /## CounterexampleCard \(PASS\)/);
  assert.match(wave5, /PR 182 merge `a464568`/);
  ...
  assert.match(campus, /PR 182 merge `a464568`/);
  assert.match(campus, /CounterexampleCard \*\*PASS\*\*/);
  assert.match(campus, /a556a0b5a2beaec4b0cc44eadcbf6c092cd152a568282ce3eee06564be01af37/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-counterexample-card\.md/);
  assert.match(src("AGENTS.md"), /CounterexampleCardDemo/);

...
const STILLS = "/opt/cursor/artifacts/remotion-audio-bed-unmute/2026-09-20";
const SLATE =
...
test("AudioBed soft-unmutes the silent fixture; no Cap A-roll", () => {
  ...
  assert.match(layers, /export const AUDIO_BED_MUTED = false/);
  assert.match(layers, /muted = AUDIO_BED_MUTED/);
  assert.match(layers, /Soft-unmuted silent fixture/);
  assert.match(layers, /No Cap A-roll/);
  assert.doesNotMatch(layers, /a_roll\.mp4|27pn9xs0zk8a73g/);
  assert.match(demo, /<AudioBed \/>/);
  assert.match(demo, /unmuted\./);
  assert.doesNotMatch(demo, /a_roll\.mp4|27pn9xs0zk8a73g/);
  assert.equal(existsSync(join(root, "public", "audio-bed-silence.wav")), true);
});

test("ORDER LOCK intact; docs seal AudioBed unmute PASS with hold_cleaning", () => {
  ...
  const note = src("antagonist-audio-bed-unmute.md");
  ...
  assert.match(note, /EDU-S04: PASS/);
  assert.match(note, /hold_cleaning: true/);
  assert.match(note, /3f42545633b331c184febe17349c9d3162295c97dabeec5458bebd2007aa95e5/);
  assert.match(note, /PR 101 merge `707a2d5af7276a3a4ec5cba272053914c23680cd`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up/,
  );
  assert.match(full, /## AudioBed unmute 2026-09-20/);
  assert.match(full, /EDU-S04: PASS/);
  assert.match(full, /Dated \*\*2026-09-20\*\* reaudit/);
  assert.match(wave5, /## AudioBed unmute \(PASS\)/);
  assert.match(wave5, /PR 101 merge `707a2d5`/);
  ...
  assert.match(campus, /PR 101 merge `707a2d5`/);
  assert.match(campus, /AudioBed unmute \*\*PASS\*\*/);
  assert.match(campus, /3f42545633b331c184febe17349c9d3162295c97dabeec5458bebd2007aa95e5/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-audio-bed-unmute\.md/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of ["AudioBedDemo-f0.png", "AudioBedDemo-f60.png", "LessonSpine-slate-f330.png"]) {

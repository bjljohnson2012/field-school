...
const STILLS = "/opt/cursor/artifacts/remotion-antagonist-checkpoint-encode-master/2026-09-21";
const PRIOR =
...
test("full-set checkpoint encode master reaudit is dated PASS with hold_cleaning", () => {
  ...
  assert.match(full, /## LessonSpine checkpoint encode master dest reaudit 2026-09-21/);
  assert.match(full, /19e5a19c47f72197bbf726707b839bdf287fd3f0111d105b2161fd694584246a/);
  ...
  assert.match(src("antagonist-checkpoint-encode-master.md"), /EDU-S01: PASS/);
  assert.match(src("antagonist-checkpoint-encode-master.md"), /EDU-S02: PASS/);
  assert.match(
    src("antagonist-checkpoint-encode-master.md"),
    /19e5a19c47f72197bbf726707b839bdf287fd3f0111d105b2161fd694584246a/,
  );
  assert.match(
    src("antagonist-checkpoint-encode-master.md"),
    /PR 145 merge `aaa7dc26ac74cd7a3827f9b8e223925e3fc631b8`/,
  );
  assert.match(wave5, /## LessonSpine checkpoint encode master dest reaudit \(PASS\)/);
  assert.match(wave5, /PR 145 merge `aaa7dc2`/);
  ...
  assert.match(campus, /PR 145 merge `aaa7dc2`/);
  assert.match(campus, /LessonSpine checkpoint encode master dest reaudit \*\*PASS\*\*/);
  assert.match(campus, /19e5a19c47f72197bbf726707b839bdf287fd3f0111d105b2161fd694584246a/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-checkpoint-encode-master\.md/);
  assert.doesNotMatch(wave5, /8\/8 PASS|launch OPEN/i);
});

test("locked checkpoint encode master dest and audit stills exist; priors untouched", () => {

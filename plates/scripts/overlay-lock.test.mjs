...
const STILLS = "/opt/cursor/artifacts/remotion-overlay-lock/2026-09-20";
const PRIOR =
...
test("OverlayLock is a logo+title lock at the seal crop", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "OverlayLockDemo.tsx");
  const lock = layers.slice(layers.indexOf("export function OverlayLock"), layers.indexOf("export function GoldRule"));
  assert.match(layers, /export const OVERLAY_LOCK_TITLE_PX/);
  assert.match(layers, /export const OVERLAY_LOCK_LETTER/);
  assert.match(layers, /export const OVERLAY_LOCK_WORD/);
  assert.match(layers, /Logo\+title lock/);
  assert.match(layers, /OVERLAY_LOCK_TITLE_PX = 30/);
  ...
  assert.match(lock, /staticFile\("isolated-seal.svg"\)/);
  assert.match(src("src", "brand.ts"), /x: 1576/);
  assert.match(src("src", "brand.ts"), /y: 24/);
  assert.match(src("src", "brand.ts"), /w: 80/);
  assert.match(src("src", "brand.ts"), /h: 64/);
  ...
  assert.match(demo, /OverlayLockDemo/);
  assert.match(demo, /logo\+title lock/);
  assert.match(demo, /<OverlayLock/);
  assert.doesNotMatch(demo, /a_roll\.mp4|27pn9xs0zk8a73g/);
});

test("OverlayLockDemo registers after CaptionDensityDemo; ORDER LOCK intact", () => {
  const rootTsx = src("src", "Root.tsx");
  const spine = src("src", "LessonSpine.tsx");
  assert.match(rootTsx, /id="OverlayLockDemo"/);
  assert.match(rootTsx, /id="CaptionDensityDemo"/);
  assert.match(rootTsx, /id="LessonSpine"/);
  assert.ok(rootTsx.indexOf('id="CaptionDensityDemo"') < rootTsx.indexOf('id="OverlayLockDemo"'));
...
test("ORDER LOCK intact; docs seal OverlayLock PASS with hold_cleaning", () => {
  const note = src("antagonist-overlay-lock.md");
  ...
  assert.match(note, /PR 111 merge `37fbe4d4f66ef6fe27e2f11ee646f297e53334fc`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up/,
  );
  assert.match(full, /## OverlayLock 2026-09-20/);
  assert.match(full, /Dated \*\*2026-09-20\*\* reaudit/);
  assert.match(wave5, /## OverlayLock \(PASS\)/);
  assert.match(wave5, /PR 111 merge `37fbe4d`/);
  ...
  assert.match(campus, /PR 111 merge `37fbe4d`/);
  assert.match(campus, /OverlayLock \*\*PASS\*\*/);
  assert.match(campus, /68b8378a47d9eb57ed4b41f35bc2bf4ed6fd01a0694fca28c0a5ea90101fee92/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-overlay-lock\.md/);
  assert.match(src("AGENTS.md"), /OverlayLockDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of ["OverlayLockDemo-f0.png", "OverlayLockDemo-f60.png", "LessonSpine-sting-f42.png"]) {

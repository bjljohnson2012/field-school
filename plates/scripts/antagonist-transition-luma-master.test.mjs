...
const STILLS = "/opt/cursor/artifacts/remotion-antagonist-transition-luma-master/2026-09-20";
const PRIOR =
...
test("full-set transition-luma master reaudit is dated PASS with hold_cleaning", () => {
  ...
  assert.match(full, /## LessonSpine transition-luma master dest reaudit 2026-09-20/);
  assert.match(full, /f631402a6dfbb2ca2218ea9617c7b2af527eb7e88f8df5547dc6584c9ef5a96e/);
  ...
  assert.match(src("antagonist-transition-luma-master.md"), /EDU-S01: PASS/);
  assert.match(src("antagonist-transition-luma-master.md"), /EDU-S02: PASS/);
  assert.match(src("antagonist-transition-luma-master.md"), /f631402a6dfbb2ca2218ea9617c7b2af527eb7e88f8df5547dc6584c9ef5a96e/);
  assert.match(src("antagonist-transition-luma-master.md"), /PR 116 merge `e5e46db240fd5251b3e3996da746f50e08697625`/);
  assert.match(wave5, /## LessonSpine transition-luma master dest reaudit \(PASS\)/);
  assert.match(wave5, /PR 116 merge `e5e46db`/);
  ...
  assert.match(campus, /PR 116 merge `e5e46db`/);
  assert.match(campus, /LessonSpine transition-luma master dest reaudit \*\*PASS\*\*/);
  assert.match(campus, /f631402a6dfbb2ca2218ea9617c7b2af527eb7e88f8df5547dc6584c9ef5a96e/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-transition-luma-master\.md/);
  assert.doesNotMatch(wave5, /8\/8 PASS|launch OPEN/i);
});

test("locked transition-luma master dest and audit stills exist; priors untouched", () => {
  ...
  for (const name of [
    ...
    "TransitionLumaDemo-f0.png",
    "TransitionLumaDemo-f9.png",
    "TransitionLumaDemo-f60.png",
  ]) {

...
const MASTER_SHA = "1e244f82eabceff7e3b16f39d41ff8d49e002aec188c694257f5da9bb4e77f17";
const STILLS = "/opt/cursor/artifacts/remotion-timeline-rail/2026-09-21";
const PRIOR =
  "/opt/cursor/artifacts/lesson-spine-caveat-encode/2026-09-21/LessonSpine.mp4";
const PRIOR_SHA = "e23cadde85af2540d3c4f9c8a8434126655133b50c0cdc6c5783b16d9e446fd7";
...
test("TimelineRail is a sequence of beats / ordered milestones after reflect", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "TimelineRailDemo.tsx");
  const recap = src("src", "RecapCard.tsx");
  const board = src("src", "DefinitionBoard.tsx");
  assert.match(layers, /export function TimelineRail/);
  assert.match(layers, /export const TIMELINE_RAIL_KICKER/);
  assert.match(layers, /export const TIMELINE_RAIL_LINE/);
  assert.match(layers, /TIMELINE_RAIL_KICKER = "Timeline"/);
  assert.match(layers, /TIMELINE_RAIL_LINE = "Mark the beats\."/);
  assert.match(layers, /TIMELINE_RAIL_ONE = "1\. Open first\."/);
  assert.match(layers, /TIMELINE_RAIL_TWO = "2\. Close last\."/);
  assert.match(layers, /Sequence of beats \/ ordered milestones/);
  assert.match(layers, /After sting\/objective path, after reflect, before practice/);
  ...
  assert.match(demo, /TimelineRailDemo/);
  assert.match(demo, /name="timeline"/);
...
test("TimelineRailDemo registers after ReflectionPromptDemo; Recap/Definition sequence timeline after reflect before practice", () => {
  ...
  assert.match(rootTsx, /id="TimelineRailDemo"/);
  assert.match(rootTsx, /id="ReflectionPromptDemo"/);
  assert.match(rootTsx, /id="LessonSpine"/);
  assert.ok(rootTsx.indexOf('id="ReflectionPromptDemo"') < rootTsx.indexOf('id="TimelineRailDemo"'));
  assert.ok(rootTsx.indexOf('id="TimelineRailDemo"') < rootTsx.indexOf('id="LessonSpine"'));
  assert.doesNotMatch(opener, /TimelineRail|name="timeline"/);
  assert.doesNotMatch(head, /TimelineRail|name="timeline"/);
  assert.doesNotMatch(quiz, /TimelineRail|name="timeline"/);
  const boardWipe = board.indexOf('name="transition"');
  const boardReflect = board.indexOf('name="reflect"');
  const boardTimeline = board.indexOf('name="timeline"');
  const boardAudio = board.indexOf('name="audio"');
  assert.ok(
    boardWipe >= 0 &&
      boardReflect > boardWipe &&
      boardTimeline > boardReflect &&
...
test("ORDER LOCK intact; docs seal TimelineRail PASS with hold_cleaning", () => {
  const note = src("antagonist-timeline-rail.md");
  ...
  assert.match(note, /1e244f82eabceff7e3b16f39d41ff8d49e002aec188c694257f5da9bb4e77f17/);
  assert.match(note, /PR 161 merge `123f60ac71d639fde6086805fbb36925cfcb036c`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## TimelineRail 2026-09-21/);
  assert.match(full, /Dated \*\*2026-09-21\*\* reaudit/);
  assert.match(wave5, /## TimelineRail \(PASS\)/);
  assert.match(wave5, /PR 161 merge `123f60a`/);
  ...
  assert.match(campus, /PR 161 merge `123f60a`/);
  assert.match(campus, /TimelineRail \*\*PASS\*\*/);
  assert.match(campus, /1e244f82eabceff7e3b16f39d41ff8d49e002aec188c694257f5da9bb4e77f17/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-timeline-rail\.md/);
  assert.match(src("AGENTS.md"), /TimelineRailDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of [
    "TimelineRailDemo-f0.png",
    "TimelineRailDemo-f60.png",
...
test("checklist on reflection-encode master holds; --flip refused", () => {

...
const STILLS = "/opt/cursor/artifacts/remotion-recapcard-slate/2026-09-20";
const EDU_S01 =
...
test("RecapCard slate kicker plus EDU-S02 Keyword gold tick", () => {
  ...
  assert.match(recap, /import \{[^}]*Keyword[^}]*\} from "\.\/layers"/);
  assert.match(recap, /<Kicker>\{kicker\}<\/Kicker>/);
  assert.match(recap, /<Keyword>\{keyword\}<\/Keyword>/);
  assert.match(recap, /words\.slice\(0, -1\)/);
  assert.match(recap, /<ObjectiveSlate objective=\{objective\}/);
  assert.match(recap, /points\.slice\(0, 3\)/);
  assert.doesNotMatch(recap, /points\.slice\(0, 4\)/);
  assert.doesNotMatch(recap, /<Title>\{title\}<\/Title>/);
  assert.match(src("src", "Root.tsx"), /kicker: "Recap"/);
  assert.match(src("src", "Root.tsx"), /title: "What stays on the card"/);
  assert.match(recap, /<LowerThird label="Recap" \/>/);
  ...
  assert.doesNotMatch(recap, /a_roll\.mp4|27pn9xs0zk8a73g/);
});

test("ORDER LOCK intact; docs seal RecapCard slate PASS with hold_cleaning", () => {
  ...
  const note = src("antagonist-recapcard-slate.md");
  ...
  assert.match(note, /PR 92 merge `b40ea28331b16d3eacb1443c08b7b90f60564a87`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up/,
  );
  assert.match(full, /## RecapCard slate 2026-09-20/);
  assert.match(full, /EDU-S02: PASS/);
  assert.match(full, /Dated \*\*2026-09-20\*\* reaudit/);
  assert.match(src("antagonist-opener-recap.md"), /## RecapCard slate 2026-09-20/);
  assert.match(wave5, /## RecapCard slate \(PASS\)/);
  assert.match(wave5, /PR 92 merge `b40ea28`/);
  ...
  assert.match(campus, /PR 92 merge `b40ea28`/);
  assert.match(campus, /RecapCard slate \*\*PASS\*\*/);
  assert.match(campus, /eaf6f84a5363b23f512c0c07f82f90464f11b8c3b57d69a0fa23af951d9798e8/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-recapcard-slate\.md/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of ["RecapCard-f144.png", "LessonSpine-recap-f864.png"]) {

...
const STILLS = "/opt/cursor/artifacts/remotion-talkinghead-slate/2026-09-20";
const EDU_S01 =
...
test("TalkingHeadCard slate kicker plus EDU-S02 Keyword gold tick", () => {
  ...
  assert.match(head, /import \{[^}]*Keyword[^}]*\} from "\.\/layers"/);
  assert.match(head, /<Kicker>Slate<\/Kicker>/);
  assert.match(head, /<Keyword>\{keyword\}<\/Keyword>/);
  assert.match(head, /words\.slice\(0, -1\)/);
  assert.match(head, /Docked, not full-bleed/);
  assert.doesNotMatch(head, /<Title>Docked, not full-bleed<\/Title>/);
  assert.doesNotMatch(head, /\{name\} · \{role\}/);
  ...
  assert.doesNotMatch(head, /a_roll\.mp4|27pn9xs0zk8a73g/);
});

test("ORDER LOCK intact; docs seal TalkingHead slate PASS with hold_cleaning", () => {
  ...
  const note = src("antagonist-talkinghead-slate.md");
  ...
  assert.match(note, /PR 91 merge `73f540186ab6a444c5d69e026c69025412406a88`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up/,
  );
  assert.match(full, /## TalkingHead slate 2026-09-20/);
  assert.match(full, /EDU-S02: PASS/);
  assert.match(full, /Dated \*\*2026-09-20\*\* reaudit/);
  assert.match(src("antagonist-definition-quiz-head.md"), /## TalkingHead slate 2026-09-20/);
  assert.match(wave5, /## TalkingHead slate \(PASS\)/);
  assert.match(wave5, /PR 91 merge `73f5401`/);
  ...
  assert.match(campus, /PR 91 merge `73f5401`/);
  assert.match(campus, /TalkingHead slate \*\*PASS\*\*/);
  assert.match(campus, /eaf6f84a5363b23f512c0c07f82f90464f11b8c3b57d69a0fa23af951d9798e8/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-talkinghead-slate\.md/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of ["TalkingHeadCard-f45.png", "TalkingHeadCard-f90.png", "LessonSpine-slate-f345.png"]) {

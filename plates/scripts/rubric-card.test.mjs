...
const MASTER_SHA = "955f0256424fdc4ddb5ae5506a04a9ea877cb6a79ccb126ba1bb4884b9943a6f";
const STILLS = "/opt/cursor/artifacts/remotion-rubric-card/2026-09-21";
const PRIOR =
  "/opt/cursor/artifacts/lesson-spine-spectrum-encode/2026-09-21/LessonSpine.mp4";
const PRIOR_SHA = "496f506babd11a6e5247e87c3bbf926117b0de31feae5b6573fa891bdf6c68d0";
...
test("RubricCard is scoring bands / criteria distinct from cutoff after threshold", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "RubricCardDemo.tsx");
  const recap = src("src", "RecapCard.tsx");
  const board = src("src", "DefinitionBoard.tsx");
  assert.match(layers, /export function RubricCard/);
  assert.match(layers, /export const RUBRIC_CARD_KICKER/);
  assert.match(layers, /export const RUBRIC_CARD_LINE/);
  assert.match(layers, /RUBRIC_CARD_KICKER = "Rubric"/);
  assert.match(layers, /RUBRIC_CARD_LINE = "Score the bands\."/);
  assert.match(layers, /RUBRIC_CARD_ONE = "A\. Holds\."/);
  assert.match(layers, /RUBRIC_CARD_TWO = "B\. Misses\."/);
  assert.match(layers, /Scoring bands \/ criteria distinct from cutoff/);
  assert.match(layers, /After sting\/objective path, after threshold, before practice/);
  assert.match(layers, /borderLeft: `6px solid \$\{gold\}`/);
  assert.match(layers, /<Keyword>\{keyword\}<\/Keyword>/);
  assert.match(layers, /<Keyword>\{oneKeyword\}<\/Keyword>/);
  assert.match(layers, /<Keyword>\{twoKeyword\}<\/Keyword>/);
  ...
  assert.match(demo, /RubricCardDemo/);
  assert.match(demo, /name="rubric"/);
...
test("RubricCardDemo registers after ThresholdCardDemo; Recap/Definition sequence rubric after threshold before practice", () => {
  ...
  assert.match(rootTsx, /id="RubricCardDemo"/);
  assert.match(rootTsx, /id="ThresholdCardDemo"/);
  assert.match(rootTsx, /id="LessonSpine"/);
  assert.ok(rootTsx.indexOf('id="ThresholdCardDemo"') < rootTsx.indexOf('id="RubricCardDemo"'));
  assert.ok(rootTsx.indexOf('id="RubricCardDemo"') < rootTsx.indexOf('id="LessonSpine"'));
  assert.doesNotMatch(opener, /RubricCard|name="rubric"/);
  assert.doesNotMatch(head, /RubricCard|name="rubric"/);
  assert.doesNotMatch(quiz, /RubricCard|name="rubric"/);
  const boardWipe = board.indexOf('name="transition"');
  const boardThreshold = board.indexOf('name="threshold"');
  const boardRubric = board.indexOf('name="rubric"');
  const boardAudio = board.indexOf('name="audio"');
  assert.ok(
    boardWipe >= 0 &&
      boardThreshold > boardWipe &&
      boardRubric > boardThreshold &&
...
test("ORDER LOCK intact; docs seal RubricCard PASS with hold_cleaning", () => {
  const note = src("antagonist-rubric-card.md");
  ...
  assert.match(note, /955f0256424fdc4ddb5ae5506a04a9ea877cb6a79ccb126ba1bb4884b9943a6f/);
  assert.match(note, /PR 173 merge `ba62e6b76597d6ad3da1d11ec58b978db17018bf`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## RubricCard 2026-09-21/);
  assert.match(full, /Dated \*\*2026-09-21\*\* reaudit/);
  assert.match(wave5, /## RubricCard \(PASS\)/);
  assert.match(wave5, /PR 173 merge `ba62e6b`/);
  assert.match(wave5, /955f0256424fdc4ddb5ae5506a04a9ea877cb6a79ccb126ba1bb4884b9943a6f/);
  assert.match(wave5, /\*\*Current master dest\.\*\*/);
  assert.match(campus, /PR 173 merge `ba62e6b`/);
  assert.match(campus, /RubricCard \*\*PASS\*\*/);
  assert.match(campus, /955f0256424fdc4ddb5ae5506a04a9ea877cb6a79ccb126ba1bb4884b9943a6f/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-rubric-card\.md/);
  assert.match(src("AGENTS.md"), /RubricCardDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of [
    "RubricCardDemo-f0.png",
    "RubricCardDemo-f60.png",
...
test("checklist on threshold-encode master holds; --flip refused", () => {

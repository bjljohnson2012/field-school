...
const STILLS = "/opt/cursor/artifacts/remotion-antagonist-quote-encode-master/2026-09-21";
const PRIOR =
...
test("full-set quote encode master reaudit is dated PASS with hold_cleaning", () => {
  ...
  assert.match(full, /## LessonSpine quote encode master dest reaudit 2026-09-21/);
  assert.match(full, /244c485bbfd83e2138c4e499fc07a00f566be3d1ede1c5a3f03dfd57b04feb2d/);
  ...
  assert.match(src("antagonist-quote-encode-master.md"), /EDU-S01: PASS/);
  assert.match(src("antagonist-quote-encode-master.md"), /EDU-S02: PASS/);
  assert.match(
    src("antagonist-quote-encode-master.md"),
    /244c485bbfd83e2138c4e499fc07a00f566be3d1ede1c5a3f03dfd57b04feb2d/,
  );
  assert.match(
    src("antagonist-quote-encode-master.md"),
    /PR 151 merge `8cd8ded2599df57373ad5d46141c9b9225132473`/,
  );
  assert.match(wave5, /## LessonSpine quote encode master dest reaudit \(PASS\)/);
  assert.match(wave5, /PR 151 merge `8cd8ded`/);
  ...
  assert.match(campus, /PR 151 merge `8cd8ded`/);
  assert.match(campus, /LessonSpine quote encode master dest reaudit \*\*PASS\*\*/);
  assert.match(campus, /244c485bbfd83e2138c4e499fc07a00f566be3d1ede1c5a3f03dfd57b04feb2d/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-quote-encode-master\.md/);
  assert.doesNotMatch(wave5, /8\/8 PASS|launch OPEN/i);
});

test("locked quote encode master dest and audit stills exist; priors untouched", () => {

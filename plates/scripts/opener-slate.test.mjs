...
const STILLS = "/opt/cursor/artifacts/remotion-opener-slate/2026-09-20";
const EDU_S01 =
...
test("Opener slate kicker plus EDU-S02 Keyword gold tick", () => {
  ...
  assert.match(opener, /import \{[^}]*Keyword[^}]*\} from "\.\/layers"/);
  assert.match(opener, /<Kicker>\{kicker\}<\/Kicker>/);
  assert.match(opener, /<Keyword>\{keyword\}<\/Keyword>/);
  assert.match(opener, /words\.slice\(0, -1\)/);
  assert.match(opener, /<ObjectiveSlate objective=\{objective\}/);
  assert.doesNotMatch(opener, /<Title>\{title\}<\/Title>/);
  assert.match(src("src", "Root.tsx"), /kicker: "Lesson"/);
  assert.match(src("src", "Root.tsx"), /title: "You Can Just Do Things"/);
  assert.match(opener, /<LowerThird label="Opener" \/>/);
  ...
  assert.doesNotMatch(opener, /a_roll\.mp4|27pn9xs0zk8a73g/);
});

test("ORDER LOCK intact; docs seal Opener slate PASS with hold_cleaning", () => {
  ...
  const note = src("antagonist-opener-slate.md");
  ...
  assert.match(note, /PR 93 merge `809ac5e54665959bfaa8c43cbacf79de29d52b6f`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up/,
  );
  assert.match(full, /## Opener slate 2026-09-20/);
  assert.match(full, /EDU-S02: PASS/);
  assert.match(full, /Dated \*\*2026-09-20\*\* reaudit/);
  assert.match(src("antagonist-opener-recap.md"), /## Opener slate 2026-09-20/);
  assert.match(wave5, /## Opener slate \(PASS\)/);
  assert.match(wave5, /PR 93 merge `809ac5e`/);
  ...
  assert.match(campus, /PR 93 merge `809ac5e`/);
  assert.match(campus, /Opener slate \*\*PASS\*\*/);
  assert.match(campus, /eaf6f84a5363b23f512c0c07f82f90464f11b8c3b57d69a0fa23af951d9798e8/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-opener-slate\.md/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of ["Opener-f42.png", "LessonSpine-sting-f42.png"]) {

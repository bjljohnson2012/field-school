...
const STILLS = "/opt/cursor/artifacts/remotion-chapter-chip/2026-09-20";
const SLATE =
...
test("ChapterChip is a beat label signal after progress", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "ChapterChipDemo.tsx");
  assert.match(layers, /export function ChapterChip/);
  assert.match(layers, /export const CHAPTER_CHIP_LABELS/);
  assert.match(layers, /sting: "Sting"/);
  assert.match(layers, /"next-up": "Next up"/);
  assert.match(layers, /top: LETTERBOX_H \+ 16/);
  assert.match(layers, /Beat label signal/);
  assert.match(layers, /After progress, before audio/);
  ...
  assert.match(demo, /ChapterChipDemo/);
  assert.match(demo, /name="chapter"/);
  assert.match(demo, /<ChapterChip beat=\{beat\} \/>/);
  assert.match(demo, /Math\.floor\(frame \/ 48\)/);
  assert.doesNotMatch(demo, /a_roll\.mp4|27pn9xs0zk8a73g/);
});

test("ChapterChipDemo registers; five spine plates carry chapter between progress and audio", () => {
  const rootTsx = src("src", "Root.tsx");
  const spine = src("src", "LessonSpine.tsx");
  assert.match(rootTsx, /id="ChapterChipDemo"/);
  ...
  for (const [file, beat] of Object.entries(plates)) {
    ...
    assert.ok(progress >= 0 && chapter > progress && audio > chapter, file);
...
test("ORDER LOCK intact; docs seal ChapterChip PASS with hold_cleaning", () => {
  const note = src("antagonist-chapter-chip.md");
  ...
  assert.match(note, /PR 103 merge `a0cc86d13f6b5b85004e74a6ae84251b47a9d1ae`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up/,
  );
  assert.match(full, /## ChapterChip 2026-09-20/);
  assert.match(full, /Dated \*\*2026-09-20\*\* reaudit/);
  assert.match(wave5, /## ChapterChip \(PASS\)/);
  assert.match(wave5, /PR 103 merge `a0cc86d`/);
  ...
  assert.match(campus, /PR 103 merge `a0cc86d`/);
  assert.match(campus, /ChapterChip \*\*PASS\*\*/);
  assert.match(campus, /3f42545633b331c184febe17349c9d3162295c97dabeec5458bebd2007aa95e5/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-chapter-chip\.md/);
  assert.match(src("AGENTS.md"), /ChapterChipDemo/);
...
test("stills exist; locked master and priors untouched", () => {
  ...
  for (const name of ["ChapterChipDemo-f0.png", "ChapterChipDemo-f60.png", "LessonSpine-slate-f330.png"]) {

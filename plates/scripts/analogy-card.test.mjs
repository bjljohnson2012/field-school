...
const MASTER_SHA = "5d0499f6cbff19149ce0b5da3c615a1ca0ab26fbbece2d9e26da0e78dd367c4b";
const STILLS = "/opt/cursor/artifacts/remotion-analogy-card/2026-09-21";
const PRIOR =
...
test("AnalogyCard is analogy / like-this transfer pair distinct from two claims and show-the-work after evidence", () => {
  const layers = src("src", "layers.tsx");
  const demo = src("src", "AnalogyCardDemo.tsx");
  const recap = src("src", "RecapCard.tsx");
  const board = src("src", "DefinitionBoard.tsx");
  assert.match(layers, /export function AnalogyCard/);
  assert.match(layers, /export const ANALOGY_CARD_KICKER/);
  assert.match(layers, /export const ANALOGY_CARD_LINE/);
  assert.match(layers, /ANALOGY_CARD_KICKER = "Analogy"/);
  assert.match(layers, /ANALOGY_CARD_LINE = "Map like this\."/);
  assert.match(layers, /ANALOGY_CARD_ONE = "Like a seed\."/);
  assert.match(layers, /ANALOGY_CARD_TWO = "So a type\."/);
  assert.match(layers, /Analogy \/ like-this transfer pair distinct from two claims and show-the-work/);
  assert.match(layers, /After sting\/objective path, after evidence, before practice/);
  ...
  assert.match(layers, /borderLeft: `10px solid \$\{gold\}`/);
  ...
  assert.match(demo, /AnalogyCardDemo/);
  assert.match(demo, /name="analogy"/);
...
test("AnalogyCardDemo registers after EvidenceCardDemo; Recap/Definition sequence analogy after evidence before practice", () => {
  ...
  assert.ok(rootTsx.indexOf('id="EvidenceCardDemo"') < rootTsx.indexOf('id="AnalogyCardDemo"'));
  assert.ok(rootTsx.indexOf('id="AnalogyCardDemo"') < rootTsx.indexOf('id="LessonSpine"'));
  assert.doesNotMatch(opener, /AnalogyCard|name="analogy"/);
  assert.doesNotMatch(head, /AnalogyCard|name="analogy"/);
  assert.doesNotMatch(quiz, /AnalogyCard|name="analogy"/);
  ...
  assert.ok(
    boardWipe >= 0 &&
      boardEvidence > boardWipe &&
      boardAnalogy > boardEvidence &&
...
test("ORDER LOCK intact; docs seal AnalogyCard PASS with hold_cleaning", () => {
  const note = src("antagonist-analogy-card.md");
  ...
  assert.match(note, /PR 179 merge `222d4de9061aa3d2e09acea68fba6117b8aed1bd`/);
  assert.match(
    note,
    /Opener\/sting → TalkingHead\/slate → DefinitionBoard\/objective → RecapCard → QuizBumper\/next-up → EndCard/,
  );
  assert.match(full, /## AnalogyCard 2026-09-21/);
  assert.match(full, /Dated \*\*2026-09-21\*\* reaudit/);
  assert.match(wave5, /## AnalogyCard \(PASS\)/);
  assert.match(wave5, /PR 179 merge `222d4de`/);
  ...
  assert.match(campus, /PR 179 merge `222d4de`/);
  assert.match(campus, /AnalogyCard \*\*PASS\*\*/);
  assert.match(campus, /5d0499f6cbff19149ce0b5da3c615a1ca0ab26fbbece2d9e26da0e78dd367c4b/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-analogy-card\.md/);
  assert.match(src("AGENTS.md"), /AnalogyCardDemo/);
...
test("checklist on evidence-encode master holds; --flip refused", () => {

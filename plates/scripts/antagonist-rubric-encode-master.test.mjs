...
const STILLS = "/opt/cursor/artifacts/remotion-antagonist-rubric-encode-master/2026-09-21";
const PRIOR =
...
test("full-set rubric encode master reaudit is dated PASS with hold_cleaning", () => {
  ...
  assert.match(full, /## LessonSpine rubric encode master dest reaudit 2026-09-21/);
  assert.match(full, /4aef4c713218247ebb4cad42b637f4b1331a02d6db4fcc13ac13e2bde156a337/);
  ...
  assert.match(src("antagonist-rubric-encode-master.md"), /EDU-S01: PASS/);
  assert.match(src("antagonist-rubric-encode-master.md"), /EDU-S02: PASS/);
  assert.match(
    src("antagonist-rubric-encode-master.md"),
    /4aef4c713218247ebb4cad42b637f4b1331a02d6db4fcc13ac13e2bde156a337/,
  );
  assert.match(
    src("antagonist-rubric-encode-master.md"),
    /PR 175 merge `d9bcd4a40397d31cd9ba7f962228df386fec9eae`/,
  );
  assert.match(wave5, /## LessonSpine rubric encode master dest reaudit \(PASS\)/);
  assert.match(wave5, /PR 175 merge `d9bcd4a`/);
  ...
  assert.match(campus, /PR 175 merge `d9bcd4a`/);
  assert.match(campus, /LessonSpine rubric encode master dest reaudit \*\*PASS\*\*/);
  assert.match(campus, /4aef4c713218247ebb4cad42b637f4b1331a02d6db4fcc13ac13e2bde156a337/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-rubric-encode-master\.md/);
  assert.doesNotMatch(wave5, /8\/8 PASS|launch OPEN/i);
});

test("locked rubric encode master dest and audit stills exist; priors untouched", () => {

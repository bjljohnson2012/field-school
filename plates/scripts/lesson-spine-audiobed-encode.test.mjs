...
test("LessonSpine plates keep AudioBed + karaoke + letterbox + OverlayLock; ORDER LOCK unchanged", () => {
  ...
  assert.match(layers, /ACTIVE_WORD_GOLD|#C4A35A|wordColor/);
...
test("render-lock dry-run forwards the audiobed dest and refuses Just", () => {
  const dir = mkdtempSync(join(tmpdir(), "audiobed-encode-lock-"));
...
test("new dest is not Just, Aug 30, or a prior LessonSpine encode", () => {
  assert.equal(existsSync(join(root, "encode-lesson-spine-audiobed.md")), true);
  ...
  assert.match(src("README.md"), /encode-lesson-spine-audiobed/);
  assert.equal(existsSync(join(root, "antagonist-lesson-spine-audiobed-encode.md")), true);
});

...
const STILLS = "/opt/cursor/artifacts/remotion-antagonist-steps-encode-master/2026-09-21";
const PRIOR =
...
test("full-set steps encode master reaudit is dated PASS with hold_cleaning", () => {
  ...
  assert.match(full, /## LessonSpine steps encode master dest reaudit 2026-09-21/);
  assert.match(full, /e0ead459d9505cd612403e8601ab1bfd9ef1acbfc3eba59d3640e19ce621935b/);
  ...
  assert.match(src("antagonist-steps-encode-master.md"), /EDU-S01: PASS/);
  assert.match(src("antagonist-steps-encode-master.md"), /EDU-S02: PASS/);
  assert.match(
    src("antagonist-steps-encode-master.md"),
    /e0ead459d9505cd612403e8601ab1bfd9ef1acbfc3eba59d3640e19ce621935b/,
  );
  assert.match(
    src("antagonist-steps-encode-master.md"),
    /PR 154 merge `e7d686228dc64241e368c8e4e6cfc6f80850ce5b`/,
  );
  assert.match(wave5, /## LessonSpine steps encode master dest reaudit \(PASS\)/);
  assert.match(wave5, /PR 154 merge `e7d6862`/);
  ...
  assert.match(campus, /PR 154 merge `e7d6862`/);
  assert.match(campus, /LessonSpine steps encode master dest reaudit \*\*PASS\*\*/);
  assert.match(campus, /e0ead459d9505cd612403e8601ab1bfd9ef1acbfc3eba59d3640e19ce621935b/);
  assert.match(campus, /\*\*CLOSED\*\*, \*\*0\/8\*\*/);
  assert.match(src("README.md"), /antagonist-steps-encode-master\.md/);
  assert.doesNotMatch(wave5, /8\/8 PASS|launch OPEN/i);
});

test("locked steps encode master dest and audit stills exist; priors untouched", () => {

# LessonSpine re-encode with CompareBoard

Locked pedagogical order: Opener/sting → TalkingHead/slate → DefinitionBoard/objective → RecapCard → QuizBumper/next-up.

New dated dest only. Bundles PR 130 CompareBoard two-claim / contrast onto the ScriptureCard master stack (PR 85–129). Factory TypeCard + AudioBed + Letterbox + OverlayLock + Karaoke packing + ProgressRail + ChapterChip + LowerThird + CalloutCard + TransitionLuma + EndCard + PracticeCard + KeyClaim + ScriptureCard + CompareBoard. LessonSpine objective beat stays DefinitionBoard. No Cap A-roll.
...
```bash
mkdir -p /opt/cursor/artifacts/lesson-spine-compare-encode/2026-09-21
node scripts/render-plate.mjs \
  --comp LessonSpine \
  --dest /opt/cursor/artifacts/lesson-spine-compare-encode/2026-09-21/LessonSpine.mp4 \

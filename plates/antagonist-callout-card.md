# Independent Antagonist v2 — CalloutCard

Bar: [../docs/remotion-vox-standards.md](../docs/remotion-vox-standards.md). Do not invent a second bar. Soft notes accepted.

Dated **2026-09-20**. CalloutCard soft craft beat. Tip / aside signal on five ORDER LOCK names (`Tip` / `Aside` copy). Craft beat on `CalloutCardDemo`. Wired on the five spine plates only, after chapter, before audio. No Cap take. Plate-level stills only.
...
After PR 107 merge `a8585c203b3186d1434430e686e27c8f7a8b2c8a`. Locked master dest `/opt/cursor/artifacts/lesson-spine-progress-chapter-encode/2026-09-20/LessonSpine.mp4` sha256 `a11e31a38774266b80a8f64e088ff4eb0b59d80bc7b710e9240b6ee076e61ce7` **untouched** (plate-level stills only). Callout sits after chapter, before audio. OverlayLock 30px / `0.02em` / `0.2em` nowrap. Checklist `hold_cleaning: true`. `--flip` refused.

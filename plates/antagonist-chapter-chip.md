# Independent Antagonist v2 — ChapterChip

Bar: [../docs/remotion-vox-standards.md](../docs/remotion-vox-standards.md). Do not invent a second bar. Soft notes accepted.

Dated **2026-09-20**. ChapterChip soft craft beat. Beat label signal on five ORDER LOCK names (`Sting` / `Slate` / `Objective` / `Recap` / `Next up`). Craft beat on `ChapterChipDemo`. Wired on the five spine plates only, after progress, before audio. No Cap take. Plate-level stills only.

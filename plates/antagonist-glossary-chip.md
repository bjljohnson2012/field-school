# Independent Antagonist v2 — GlossaryChip

Bar: [../docs/remotion-vox-standards.md](../docs/remotion-vox-standards.md). Do not invent a second bar. Soft notes accepted.

Dated **2026-09-21**. GlossaryChip soft craft beat. Term / definition pop (`GLOSSARY_CHIP_KICKER = "Glossary"`; `GLOSSARY_CHIP_TERM = "Type"`; `GLOSSARY_CHIP_LINE = "Define the term."`; cream + gold 6px left rail; EDU-S02 gold tick on “term.”). Craft beat on `GlossaryChipDemo`. Sequenced after sting/objective path, after SectionTitle, before PracticeCard. GlossaryChip path only. No Cap take. Plate-level stills only.
...
After PR 135 merge `a7facba844e00865d330bcd8ea812befdc5b9ab1`. Locked master dest `/opt/cursor/artifacts/lesson-spine-section-encode/2026-09-21/LessonSpine.mp4` sha256 `f4bf345d6c0ec8c51941b3bb2d08c71589bc822632d935dc26b6d18c07a631e7` **untouched** (plate-level stills only). Checklist `hold_cleaning: true`. `--flip` refused.

## SOFT notes only

- `EDU-S01` — **PASS** on the locked master
- `EDU-S02` — **PASS** karaoke gold ticks unchanged; GlossaryChip last-word “term.” gold-ticked

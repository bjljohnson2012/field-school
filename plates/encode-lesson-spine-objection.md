# LessonSpine re-encode with ObjectionCard

Locked pedagogical order: Opener/sting → TalkingHead/slate → DefinitionBoard/objective → RecapCard → QuizBumper/next-up.

New dated dest only. Bundles PR 139 ObjectionCard steelman / respond onto the GlossaryChip master stack (PR 85–138). Factory TypeCard + AudioBed + Letterbox + OverlayLock + Karaoke packing + ProgressRail + ChapterChip + LowerThird + CalloutCard + TransitionLuma + EndCard + PracticeCard + KeyClaim + ScriptureCard + CompareBoard + SectionTitle + GlossaryChip + ObjectionCard. LessonSpine objective beat stays DefinitionBoard. No Cap A-roll.
...
```bash
mkdir -p /opt/cursor/artifacts/lesson-spine-objection-encode/2026-09-21
node scripts/render-plate.mjs \
  --comp LessonSpine \
  --dest /opt/cursor/artifacts/lesson-spine-objection-encode/2026-09-21/LessonSpine.mp4 \

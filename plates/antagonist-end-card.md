# Independent Antagonist v2 — EndCard

Bar: [../docs/remotion-vox-standards.md](../docs/remotion-vox-standards.md). Do not invent a second bar. Soft notes accepted.

Dated **2026-09-20**. EndCard soft craft beat. Lesson close signal (`END_CARD_KICKER = "Close"`; `END_CARD_LINE = "The lesson holds."`; cream + gold 6px left rail; EDU-S02 gold tick on “holds.”). Craft beat on `EndCardDemo`. Sequenced after QuizBumper/next-up, before audio. EndCard path only. No Cap take. Plate-level stills only.
...
ORDER LOCK intact: Opener/sting → TalkingHead/slate → DefinitionBoard/objective → RecapCard → QuizBumper/next-up → EndCard.

After PR 117 merge `37d7fded486f027974599daabe71ebb7ce7c126f`. Locked master dest `/opt/cursor/artifacts/lesson-spine-transition-luma-encode/2026-09-20/LessonSpine.mp4` sha256 `f631402a6dfbb2ca2218ea9617c7b2af527eb7e88f8df5547dc6584c9ef5a96e` **untouched** (plate-level stills only). Checklist `hold_cleaning: true`. `--flip` refused.

## SOFT notes only

- `EDU-S01` — **PASS** on the locked master
- `EDU-S02` — **PASS** karaoke gold ticks unchanged; EndCard last-word “holds.” gold-ticked

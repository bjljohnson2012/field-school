# Independent Antagonist v2 — RecapCard slate

Bar: [../docs/remotion-vox-standards.md](../docs/remotion-vox-standards.md). Do not invent a second bar. Soft notes accepted.

Dated **2026-09-20**. RecapCard slate-beat soft polish. Kicker “Recap” plus Title last-word gold `#C4A35A` tick (`Keyword`) on fixture “What stays on the card”. ObjectiveSlate still returns `draw one idea per beat` (EDU-S01). No Cap take. Plate-level stills only.
...
After PR 92 merge `b40ea28331b16d3eacb1443c08b7b90f60564a87`. Locked master dest `/opt/cursor/artifacts/lesson-spine-edu-s02-encode/2026-09-20/LessonSpine.mp4` sha256 `eaf6f84a5363b23f512c0c07f82f90464f11b8c3b57d69a0fa23af951d9798e8` **untouched** (plate-level stills only). Three recap points only. OverlayLock 30px / `0.02em` / `0.2em` nowrap. Checklist `hold_cleaning: true`. `--flip` refused.

## SOFT notes only

- `EDU-S01` — **PASS** on the locked master; Recap returns the same objective
- `EDU-S02` — **PASS** gold keyword tick on RecapCard Title last word + kicker “Recap”

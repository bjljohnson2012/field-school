# Independent Antagonist v2 — ProgressRail

Bar: [../docs/remotion-vox-standards.md](../docs/remotion-vox-standards.md). Do not invent a second bar. Soft notes accepted.

Dated **2026-09-20**. ProgressRail soft craft beat. Lesson position signal on five ORDER LOCK ticks (`sting` / `slate` / `objective` / `recap` / `next-up`). Craft beat on `ProgressRailDemo`. Wired on the five spine plates only, after letterbox, before audio. No Cap take. Plate-level stills only.
...
After PR 102 merge `684a98a2dd595591bf775f1cccb04ba3de20f82d`. Locked master dest `/opt/cursor/artifacts/lesson-spine-typecard-objectiveslate-encode/2026-09-20/LessonSpine.mp4` sha256 `3f42545633b331c184febe17349c9d3162295c97dabeec5458bebd2007aa95e5` **untouched** (plate-level stills only). Progress sits after letterbox, before audio. OverlayLock 30px / `0.02em` / `0.2em` nowrap. Checklist `hold_cleaning: true`. `--flip` refused.

...
Launch SoT: [../prelaunch/LAUNCH_GATE.md](../prelaunch/LAUNCH_GATE.md) — **CLOSED**, **0/8**. Do not invent **8/8**. Launch is never claimed until every node in that file is explicitly **PASS**. This readout is **0/8**.

Live roster SoT: Notion Org Chart https://app.notion.com/p/3dffe86f6dee8190b990cab34b0fc518
Do not invent seats. Do not hire Dean. Do not spawn new Product / Marketer / Revenue agents. Do not rewrite roster seats. Chief Decision Maker already owns the outer loop.

Clocks: [ROUTINES.md](./ROUTINES.md) — 06 / 09 / 15 / 22 / 02 ET. Every clock is a handoff.
...
Wire: **CDM → CTO → Cursor Gate → Field School PM**.
...
Hands rule: Cursor Gate alone contacts Field School PM / Cursor cloud agents. CTO briefs Cursor Gate. CDM briefs CTO. CDM never messages Field School PM directly.
...
- `hold_cleaning: true`. No Cleaning / Publish / Distribute flip.
- No Cap take. No Just remake (`27pn9xs0zk8a73g`). No second melt while `render.lock` exists.
- Failures escalate CDM → CTO → Cursor Gate. Do not soft-ship.

## Launch nodes → owning seats

Same seats as the locked hub. Do not invent a second org chart.
...
Sunday: CDM scores all eight nodes. Twenty lines. Contradictions listed. Launch stays **CLOSED**. This readout is **0/8**.
...
- Do not invent agents. Do not invent 8/8.

...
- One room per ticket. Two streams max.
- Build in `app/`. Do not extend frozen TanStack `src/`.
- AUTH_URL / public portal flip stays held until Ben says so (PR 31).
- No ads, no outbound, no LAUNCH until the gate is 8/8.
- Do not remessage Wave 2 as if it were unfinished identity work.
- Do not touch CNC vault `2.24.64.248`.
- Do not re-render Just (`27pn9xs0zk8a73g`).
- No second melt while `render.lock` exists.
- Public Distribute on the marketing site stays held.
- No Remotion-in-Next. Remotion stays in `plates/`.
- Wave3 campus pack LIVE (composer/teach). PR 65 stays unmerged (SUPERSEDED / held).
